...
- **Pin 0** target_time_freq_support (type: ['time_freq_support']) (optional: False): List of time freq support to potentially split in chunks.
...
- **Pin 7** abstract_meshed_region (type: ['abstract_meshed_region', 'field']) (optional: False): When a mesh is provided, the number of nodes (for "Nodal" results) or number of elements (for "Elemental" results) is used to compute the chunk. When a field is provided, its scoping size is used to compute the chunk.
