# Libraries {#libraries .sone-topic}

Scade One provides a set of libraries that can be used for model design for simulation and debug of models.

|Abbreviation|Definition|
|------------|----------|
|I|Input variable|
|O|Output variable|
|P|Size parameter|

-   **[Bitfield](../../techdoc_en-US/libraries/bitfield/bitfield.md)**  
The BitField library provides a set of operations on arrays of bits or integers seen as array of bits, with conversion from/to array of Boolean values. It also provides operators to make/split bit-fields from/to bytes.
-   **[Check](../../techdoc_en-US/libraries/check/check.md)**  
The Check library provides a set of operators to build oracles for test harnesses, which are divided into several categories.
-   **[Digital](../../techdoc_en-US/libraries/digital/digital.md)**  
The Digital library provides a set of basic functions aiming to help the construction of more complex digital data treatment algorithms.
-   **[Float](../../techdoc_en-US/libraries/float/float.md)**  
The Float library ensures good practices in the use of floating-point numbers.
-   **[Flows](../../techdoc_en-US/libraries/flows/flows.md)**  
The Flows library provides a set of simple operators on flows \(delays, counters, timers, etc\).
-   **[Math::Common](../../techdoc_en-US/libraries/common/math_common.md)**  
Common mathematical operations.
-   **[Math::Complex](../../techdoc_en-US/libraries/complex/math_complex.md)**  
Type and operations on complex numbers.
-   **[Math::Matrix](../../techdoc_en-US/libraries/matrix/math_matrix.md)**  
The Matrix library provides common operations for matrices.
-   **[Math::Trigo](../../techdoc_en-US/libraries/trigo/math_trigo.md)**  
Common trigonometric functions and their inverse.
-   **[Math::Vector](../../techdoc_en-US/libraries/vector/math_vector.md)**  
The Vector library provides common operations for vectors.
-   **[Sources](../../techdoc_en-US/libraries/sources/sources.md)**  
The Sources library provides a set of flow generators, mostly numeric, for usage in test harnesses.

**Previous topic:**[Optimization of Fast Exponentiation for Embedded Systems](../../techdoc_en-US/examples/fast_exponential/optimization_of_fast_exponentiation_for_embedded_systems.md)

**Next topic:**[Bitfield](../../techdoc_en-US/libraries/bitfield/bitfield.md)

**Previous topic:**[PyScadeOne API](../../techdoc_en-US/API/pyscadeone_api.md)

