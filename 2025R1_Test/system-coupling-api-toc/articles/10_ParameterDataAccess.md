...
Since each parameter stores only a single value, the participant library does not provide access via shared memory (as is done with [Heavyweight Data](11_HeavyweightDataAccess.md)). Instead, the participant library provides access via direct get and set calls for the parameter data.
