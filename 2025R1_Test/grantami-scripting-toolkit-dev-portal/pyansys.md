...
Python APIs. This is in contrast to the MI Scripting Toolkit, which is a monolithic, closed-source
...
<a id="interoperability-between-pyansys-packages-and-the-mi-scripting-toolkit"></a>

## Interoperability between PyAnsys packages and the MI Scripting Toolkit
...
are not directly interoperable with the MI Scripting Toolkit. Furthermore, since the MI Scripting
Toolkit is a closed-source, C++ based Python package, PyAnsys packages cannot interact directly with
MI Scripting Toolkit objects. Instead, methods will be added to the MI Scripting Toolkit to enable
...
An additional reason for formally separating the new PyAnsys packages from the existing MI Scripting
...
[requests](https://requests.readthedocs.io/en/latest/) package to communicate with the server. This is contrasted with the MI Scripting Toolkit,
which instead uses closed-source binaries. To avoid the MI Scripting Toolkit depending on both the
