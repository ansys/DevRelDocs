...
This class is to represent the Axle.

```csharp
public class Axle : LinkContainer, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public BoltingType BoltingType { get; set; }
```
...
```csharp
public ExpressionValueVariable FrontInactiveLength { get; set; }
```
...
```csharp
public ExpressionValueVariable RearInactiveLength { get; set; }
```
...
```csharp
public ExpressionValueVariable UBoltMass { get; set; }
```
