# <a id="VM_Managed_DAFUL_ConnectorMarkerTransformBase"></a> Class ConnectorMarkerTransformBase
...
```csharp
public abstract class ConnectorMarkerTransformBase : LinkContainer, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public ConnectorMarkerTransformBase(IOwned owner)
```
...
```csharp
public abstract TMatrix CalcMarkerTransform(ObjectBase ob, ObjectBase obSupport)
```
...
The support object(Body) to calculate transformation matirx.
