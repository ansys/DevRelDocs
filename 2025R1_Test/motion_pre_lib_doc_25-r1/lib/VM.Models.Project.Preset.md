# <a id="VM_Models_Project_Preset"></a> Namespace VM.Models.Project.Preset

### Classes

 [AnalysisConditionStep](VM.Models.Project.Preset.AnalysisConditionStep.md)

 [AnalysisScenarioBase](VM.Models.Project.Preset.AnalysisScenarioBase.md)

 [AnalysisStep](VM.Models.Project.Preset.AnalysisStep.md)

 [AnimationStep](VM.Models.Project.Preset.AnimationStep.md)

 [BodyStep](VM.Models.Project.Preset.BodyStep.md)

 [ChartStep](VM.Models.Project.Preset.ChartStep.md)

 [ConnectorStep](VM.Models.Project.Preset.ConnectorStep.md)

 [DrivetrainStep](VM.Models.Project.Preset.DrivetrainStep.md)

 [DynamicAnalysis](VM.Models.Project.Preset.DynamicAnalysis.md)

 [EigenvalueAnalysis](VM.Models.Project.Preset.EigenvalueAnalysis.md)

 [ExecutePostprocessorStepItem](VM.Models.Project.Preset.ExecutePostprocessorStepItem.md)

 [ExecutePreprocessorStepItem](VM.Models.Project.Preset.ExecutePreprocessorStepItem.md)

 [GeometryStep](VM.Models.Project.Preset.GeometryStep.md)

 [HeatDeflectionAnalysis](VM.Models.Project.Preset.HeatDeflectionAnalysis.md)

 [PostprocessorAnalysisStep](VM.Models.Project.Preset.PostprocessorAnalysisStep.md)

 [PostprocessorFatigueAnalysisStep](VM.Models.Project.Preset.PostprocessorFatigueAnalysisStep.md)

 [PresetHelper](VM.Models.Project.Preset.PresetHelper.md)

 [ResultStep](VM.Models.Project.Preset.ResultStep.md)

 [SharedModelingDataStep](VM.Models.Project.Preset.SharedModelingDataStep.md)

 [StiffnessAnalysis](VM.Models.Project.Preset.StiffnessAnalysis.md)

 [StrengthAnalysis](VM.Models.Project.Preset.StrengthAnalysis.md)

 [SystemModelingStep](VM.Models.Project.Preset.SystemModelingStep.md)

 [ThermalStep](VM.Models.Project.Preset.ThermalStep.md)

