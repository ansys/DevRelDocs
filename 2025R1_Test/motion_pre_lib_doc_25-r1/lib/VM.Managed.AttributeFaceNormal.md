...
```csharp
public class AttributeFaceNormal : AttributeBase, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public AttributeFaceNormal(bool bOpposite)
```

#### Parameters

`bOpposite` [bool](https://learn.microsoft.com/dotnet/api/system.boolean)
