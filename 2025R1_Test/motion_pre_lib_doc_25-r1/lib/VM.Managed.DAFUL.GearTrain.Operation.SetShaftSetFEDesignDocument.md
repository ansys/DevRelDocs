# <a id="VM_Managed_DAFUL_GearTrain_Operation_SetShaftSetFEDesignDocument"></a> Class SetShaftSetFEDesignDocument
...
```csharp
public class SetShaftSetFEDesignDocument : SetFEDesignDocument, IOperation
```
...
```csharp
public SetShaftSetFEDesignDocument(IMeshEntity meshEntity)
```
...
```csharp
public SetShaftSetFEDesignDocument(JournalParameter jp)
```
