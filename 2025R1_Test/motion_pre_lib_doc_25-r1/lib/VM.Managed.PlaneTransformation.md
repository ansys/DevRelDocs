# <a id="VM_Managed_PlaneTransformation"></a> Class PlaneTransformation
...
```csharp
public class PlaneTransformation : PlaneBase, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
[PlaneBase.TransformationMatrix](VM.Managed.PlaneBase.md\#VM\_Managed\_PlaneBase\_TransformationMatrix), 
...
```csharp
public PlaneTransformation(TransformMatrix trans)
```

#### Parameters

`trans` [TransformMatrix](VM.Managed.TransformMatrix.md)
