# SolidBody.IsRepresentedAsBSurface Property 
 

Gets or sets a value indicating whether this instance is represented as Bsurface.

**Namespace:**&nbsp;<a href="8790c1b2-b85d-f406-6b72-1668fe47b24f">VM.Managed.DAFUL</a><br />**Assembly:**&nbsp;VMDFBase (in VMDFBase.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public bool IsRepresentedAsBSurface { get; set; }
```

**VB**<br />
``` VB
Public Property IsRepresentedAsBSurface As Boolean
	Get
	Set
```

**C++**<br />
``` C++
public:
property bool IsRepresentedAsBSurface {
	bool get ();
	void set (bool value);
}
```

**F#**<br />
``` F#
member IsRepresentedAsBSurface : bool with get, set

```


#### Property Value
Type: Boolean<br />`true` if this instance is represented as Bsurface; otherwise, `false`.

## See Also


#### Reference
<a href="1fe26b85-134a-9d7b-d7d0-ab1f9b2b1cf6">SolidBody Class</a><br /><a href="8790c1b2-b85d-f406-6b72-1668fe47b24f">VM.Managed.DAFUL Namespace</a><br />