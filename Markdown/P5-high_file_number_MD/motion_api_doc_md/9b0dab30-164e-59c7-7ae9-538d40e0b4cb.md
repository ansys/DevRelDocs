# DocInfo.IsFailedLicense Property 
 

Gets a value indicating whether this instance's license is failed.

**Namespace:**&nbsp;<a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre</a><br />**Assembly:**&nbsp;dfpredat (in dfpredat.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
public bool IsFailedLicense { get; set; }
```

**VB**<br />
``` VB
Public Property IsFailedLicense As Boolean
	Get
	Set
```

**C++**<br />
``` C++
public:
property bool IsFailedLicense {
	bool get ();
	void set (bool value);
}
```

**F#**<br />
``` F#
member IsFailedLicense : bool with get, set

```


#### Property Value
Type: Boolean<br />`true` if this instance's license is failed; otherwise, `false`.

## See Also


#### Reference
<a href="7b4c77d2-d2ea-2468-d74f-462ea284de16">DocInfo Class</a><br /><a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre Namespace</a><br />