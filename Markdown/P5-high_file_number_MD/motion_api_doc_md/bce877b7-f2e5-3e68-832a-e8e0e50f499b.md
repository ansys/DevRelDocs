# ShoeProfileParametric.ShoeDistance1 Property 
 

Gets and sets longitudinal distance from reference position to shoe head.

**Namespace:**&nbsp;<a href="016cb8c1-ea7a-0630-fdec-3227772767a2">VM.Managed.CAD.Track</a><br />**Assembly:**&nbsp;VMDTrackBase (in VMDTrackBase.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public double ShoeDistance1 { get; set; }
```

**VB**<br />
``` VB
Public Property ShoeDistance1 As Double
	Get
	Set
```

**C++**<br />
``` C++
public:
property double ShoeDistance1 {
	double get ();
	void set (double value);
}
```

**F#**<br />
``` F#
member ShoeDistance1 : float with get, set

```


#### Property Value
Type: Double

## See Also


#### Reference
<a href="014bb490-60db-99fb-67de-9211e5ef6025">ShoeProfileParametric Class</a><br /><a href="016cb8c1-ea7a-0630-fdec-3227772767a2">VM.Managed.CAD.Track Namespace</a><br />