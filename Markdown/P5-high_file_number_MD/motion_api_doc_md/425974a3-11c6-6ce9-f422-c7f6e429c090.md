# Api.CloseAllFileExceptWork Method 
 

Closes all file except work.

**Namespace:**&nbsp;<a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre</a><br />**Assembly:**&nbsp;dfpreAPI (in dfpreAPI.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
public void CloseAllFileExceptWork()
```

**VB**<br />
``` VB
Public Sub CloseAllFileExceptWork
```

**C++**<br />
``` C++
public:
virtual void CloseAllFileExceptWork() sealed
```

**F#**<br />
``` F#
abstract CloseAllFileExceptWork : unit -> unit 
override CloseAllFileExceptWork : unit -> unit 
```


#### Implements
<a href="df9e1208-f39a-1ef9-bdf4-074e5cfeffd1">IPre.CloseAllFileExceptWork()</a><br />

## See Also


#### Reference
<a href="781c391e-0ecb-26c9-a34d-05f715b7d907">Api Class</a><br /><a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre Namespace</a><br />