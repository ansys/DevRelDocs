# PropertyGearSetBase.ToleranceProperty Property 
 

Gets or sets the tolerance property.

**Namespace:**&nbsp;<a href="faae22d6-2802-465b-a817-230732284684">VM.Managed.DAFUL.GearTrain</a><br />**Assembly:**&nbsp;VMDGearTrain (in VMDGearTrain.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public Tolerance ToleranceProperty { get; set; }
```

**VB**<br />
``` VB
Public Property ToleranceProperty As Tolerance
	Get
	Set
```

**C++**<br />
``` C++
public:
property Tolerance^ ToleranceProperty {
	Tolerance^ get ();
	void set (Tolerance^ value);
}
```

**F#**<br />
``` F#
member ToleranceProperty : Tolerance with get, set

```


#### Property Value
Type: <a href="8260924e-da61-c0e9-5900-e88808d5ae20">Tolerance</a><br />The tolerance property.

## See Also


#### Reference
<a href="9ae01eaa-e074-8246-5d57-25fc4f8af724">PropertyGearSetBase Class</a><br /><a href="faae22d6-2802-465b-a817-230732284684">VM.Managed.DAFUL.GearTrain Namespace</a><br />