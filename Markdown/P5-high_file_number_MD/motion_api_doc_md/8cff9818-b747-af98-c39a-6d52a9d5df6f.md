# Pre.OnActiveDocumentChanged Event
 

Occurs when [on active document changed].

**Namespace:**&nbsp;<a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre</a><br />**Assembly:**&nbsp;dfpredat (in dfpredat.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
public static event EventHandler OnActiveDocumentChanged
```

**VB**<br />
``` VB
Public Shared Event OnActiveDocumentChanged As EventHandler
```

**C++**<br />
``` C++
public:
static  event EventHandler^ OnActiveDocumentChanged {
	void add (EventHandler^ value);
	void remove (EventHandler^ value);
}
```

**F#**<br />
``` F#
member OnActiveDocumentChanged : IEvent<EventHandler,
    EventArgs>

```


#### Value
Type: System.EventHandler

## See Also


#### Reference
<a href="7453fc5e-42dc-5201-844c-19f8c701cefd">Pre Class</a><br /><a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre Namespace</a><br />