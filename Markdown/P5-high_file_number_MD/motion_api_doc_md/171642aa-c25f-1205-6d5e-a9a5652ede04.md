# Settings.AxisLocation Property 
 

Gets or sets the Axis Location.

**Namespace:**&nbsp;<a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre</a><br />**Assembly:**&nbsp;dfpredat (in dfpredat.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
public AxisLocation AxisLocation { get; set; }
```

**VB**<br />
``` VB
Public Property AxisLocation As AxisLocation
	Get
	Set
```

**C++**<br />
``` C++
public:
property AxisLocation AxisLocation {
	AxisLocation get ();
	void set (AxisLocation value);
}
```

**F#**<br />
``` F#
member AxisLocation : AxisLocation with get, set

```


#### Property Value
Type: <a href="0fb8bbbb-87f6-141b-2400-64a7258eeaa9">AxisLocation</a><br />The Axis Location.

## See Also


#### Reference
<a href="67de487e-0228-b2ad-6f64-6d2b5653647d">Settings Class</a><br /><a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre Namespace</a><br />