# BallPlacing.SynchronizeWithGeometry Property 
 

**Namespace:**&nbsp;<a href="8790c1b2-b85d-f406-6b72-1668fe47b24f">VM.Managed.DAFUL</a><br />**Assembly:**&nbsp;VMDBallPlacing (in VMDBallPlacing.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public bool SynchronizeWithGeometry { get; set; }
```

**VB**<br />
``` VB
Public Property SynchronizeWithGeometry As Boolean
	Get
	Set
```

**C++**<br />
``` C++
public:
property bool SynchronizeWithGeometry {
	bool get ();
	void set (bool value);
}
```

**F#**<br />
``` F#
member SynchronizeWithGeometry : bool with get, set

```


#### Property Value
Type: Boolean

## See Also


#### Reference
<a href="1e4f10d0-7e28-a781-6545-ed76eb9761f8">BallPlacing Class</a><br /><a href="8790c1b2-b85d-f406-6b72-1668fe47b24f">VM.Managed.DAFUL Namespace</a><br />