# SplineFile.SelectedSplineName Property 
 

Gets or sets the name of the selected spline.

**Namespace:**&nbsp;<a href="8790c1b2-b85d-f406-6b72-1668fe47b24f">VM.Managed.DAFUL</a><br />**Assembly:**&nbsp;VMDSB (in VMDSB.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public string SelectedSplineName { get; set; }
```

**VB**<br />
``` VB
Public Property SelectedSplineName As String
	Get
	Set
```

**C++**<br />
``` C++
public:
property String^ SelectedSplineName {
	String^ get ();
	void set (String^ value);
}
```

**F#**<br />
``` F#
member SelectedSplineName : string with get, set

```


#### Property Value
Type: String<br />The name of the selected spline.

## See Also


#### Reference
<a href="df51da34-3420-5fad-a3b6-df271a39b4c8">SplineFile Class</a><br /><a href="8790c1b2-b85d-f406-6b72-1668fe47b24f">VM.Managed.DAFUL Namespace</a><br />