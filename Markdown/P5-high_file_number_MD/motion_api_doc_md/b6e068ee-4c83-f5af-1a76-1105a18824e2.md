# EigenvalueAnalysisParameter.PenaltyVal Property 
 

Gets or sets the penalty value.

**Namespace:**&nbsp;<a href="05a7f2f9-3551-60b1-3652-371a61dfb10b">VM.Managed</a><br />**Assembly:**&nbsp;VMObjBase (in VMObjBase.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public double PenaltyVal { get; set; }
```

**VB**<br />
``` VB
Public Property PenaltyVal As Double
	Get
	Set
```

**C++**<br />
``` C++
public:
property double PenaltyVal {
	double get ();
	void set (double value);
}
```

**F#**<br />
``` F#
member PenaltyVal : float with get, set

```


#### Property Value
Type: Double<br />The penalty value.

## See Also


#### Reference
<a href="80154ae6-edca-f2b1-5f8e-38c67187f9c7">EigenvalueAnalysisParameter Class</a><br /><a href="05a7f2f9-3551-60b1-3652-371a61dfb10b">VM.Managed Namespace</a><br />