# Pre.Core Property 
 

Gets or sets the core.

**Namespace:**&nbsp;<a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre</a><br />**Assembly:**&nbsp;dfpredat (in dfpredat.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
public static IPre Core { set; }
```

**VB**<br />
``` VB
Public Shared WriteOnly Property Core As IPre
	Set
```

**C++**<br />
``` C++
public:
static property IPre^ Core {
	void set (IPre^ value);
}
```

**F#**<br />
``` F#
static member Core : IPre with set

```


#### Property Value
Type: <a href="97afc7c3-8294-c57e-df0a-3ab0c43aef65">IPre</a><br />The core.

## See Also


#### Reference
<a href="7453fc5e-42dc-5201-844c-19f8c701cefd">Pre Class</a><br /><a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre Namespace</a><br />