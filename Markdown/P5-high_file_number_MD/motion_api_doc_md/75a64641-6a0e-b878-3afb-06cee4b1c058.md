# Settings.RotateTrackball Property 
 

Gets or sets the RotateTrackball

**Namespace:**&nbsp;<a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre</a><br />**Assembly:**&nbsp;dfpredat (in dfpredat.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
public bool RotateTrackball { get; set; }
```

**VB**<br />
``` VB
Public Property RotateTrackball As Boolean
	Get
	Set
```

**C++**<br />
``` C++
public:
property bool RotateTrackball {
	bool get ();
	void set (bool value);
}
```

**F#**<br />
``` F#
member RotateTrackball : bool with get, set

```


#### Property Value
Type: Boolean<br />True if view rotation type is trackball type.

## See Also


#### Reference
<a href="67de487e-0228-b2ad-6f64-6d2b5653647d">Settings Class</a><br /><a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre Namespace</a><br />