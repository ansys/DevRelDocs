# Configuration.SimulationStepForPLT Property 
 

Gets or sets the simulation step of plt.

**Namespace:**&nbsp;<a href="1c5a1fc2-0c5b-dd04-9702-e89ce745554d">VM.Managed.Simulation</a><br />**Assembly:**&nbsp;VMDFBase (in VMDFBase.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public uint SimulationStepForPLT { get; set; }
```

**VB**<br />
``` VB
Public Property SimulationStepForPLT As UInteger
	Get
	Set
```

**C++**<br />
``` C++
public:
property unsigned int SimulationStepForPLT {
	unsigned int get ();
	void set (unsigned int value);
}
```

**F#**<br />
``` F#
member SimulationStepForPLT : uint32 with get, set

```


#### Property Value
Type: UInt32<br />The simulation step of plt.

## See Also


#### Reference
<a href="24984e9d-bf3b-2fe0-3653-f2061fc4ec3b">Configuration Class</a><br /><a href="1c5a1fc2-0c5b-dd04-9702-e89ce745554d">VM.Managed.Simulation Namespace</a><br />