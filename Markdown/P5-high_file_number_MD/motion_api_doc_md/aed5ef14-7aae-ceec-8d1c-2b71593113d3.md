# BeamGroup.BeamInfos Property 
 

List container of beam information objects

**Namespace:**&nbsp;<a href="cd24f896-bcad-1465-253c-e33e6c09f550">VM.Managed.DAFUL.Beam</a><br />**Assembly:**&nbsp;VMDBeamGroup (in VMDBeamGroup.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public BeamInfo[] BeamInfos { get; set; }
```

**VB**<br />
``` VB
Public Property BeamInfos As BeamInfo()
	Get
	Set
```

**C++**<br />
``` C++
public:
property array<BeamInfo^>^ BeamInfos {
	array<BeamInfo^>^ get ();
	void set (array<BeamInfo^>^ value);
}
```

**F#**<br />
``` F#
member BeamInfos : BeamInfo[] with get, set

```


#### Property Value
Type: <a href="83f9e3bb-d0bc-2c7b-7942-55b26f4ad082">BeamInfo</a>[]

## See Also


#### Reference
<a href="fe45f1f7-f879-bc72-973c-2c0b98c4ae38">BeamGroup Class</a><br /><a href="cd24f896-bcad-1465-253c-e33e6c09f550">VM.Managed.DAFUL.Beam Namespace</a><br />