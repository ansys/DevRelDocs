# Pre.GlobalJournal Property 
 

Gets the global journal.

**Namespace:**&nbsp;<a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre</a><br />**Assembly:**&nbsp;dfpredat (in dfpredat.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
public static JournalRecorder GlobalJournal { get; }
```

**VB**<br />
``` VB
Public Shared ReadOnly Property GlobalJournal As JournalRecorder
	Get
```

**C++**<br />
``` C++
public:
static property JournalRecorder^ GlobalJournal {
	JournalRecorder^ get ();
}
```

**F#**<br />
``` F#
static member GlobalJournal : JournalRecorder with get

```


#### Property Value
Type: JournalRecorder<br />The global journal.

## See Also


#### Reference
<a href="7453fc5e-42dc-5201-844c-19f8c701cefd">Pre Class</a><br /><a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre Namespace</a><br />