# BearingDimensionBase.Bore Property 
 

Gets or sets the bore.

**Namespace:**&nbsp;<a href="faae22d6-2802-465b-a817-230732284684">VM.Managed.DAFUL.GearTrain</a><br />**Assembly:**&nbsp;VMDGearTrain (in VMDGearTrain.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public ExpressionValueVariable Bore { get; set; }
```

**VB**<br />
``` VB
Public Property Bore As ExpressionValueVariable
	Get
	Set
```

**C++**<br />
``` C++
public:
property ExpressionValueVariable Bore {
	ExpressionValueVariable get ();
	void set (ExpressionValueVariable value);
}
```

**F#**<br />
``` F#
member Bore : ExpressionValueVariable with get, set

```


#### Property Value
Type: ExpressionValueVariable<br />The bore.

## See Also


#### Reference
<a href="c26a4be8-a15c-e110-237e-123d35d60770">BearingDimensionBase Class</a><br /><a href="faae22d6-2802-465b-a817-230732284684">VM.Managed.DAFUL.GearTrain Namespace</a><br />