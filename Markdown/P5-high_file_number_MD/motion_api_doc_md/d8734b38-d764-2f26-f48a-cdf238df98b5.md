# DesignParameterInfo.ChangeTargetValue Method 
 

**Namespace:**&nbsp;<a href="22f87d6f-61ec-95e1-316f-16f05c16330e">VM.Managed.DAFUL.DOE</a><br />**Assembly:**&nbsp;VMDDOE (in VMDDOE.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public bool ChangeTargetValue(
	double dValue
)
```

**VB**<br />
``` VB
Public Function ChangeTargetValue ( 
	dValue As Double
) As Boolean
```

**C++**<br />
``` C++
public:
bool ChangeTargetValue(
	double dValue
)
```

**F#**<br />
``` F#
member ChangeTargetValue : 
        dValue : float -> bool 

```


#### Parameters
&nbsp;<dl><dt>dValue</dt><dd>Type: System.Double<br /></dd></dl>

#### Return Value
Type: Boolean

## See Also


#### Reference
<a href="dd1caa2c-13a6-ca7b-07a0-ee1b910c12b7">DesignParameterInfo Class</a><br /><a href="22f87d6f-61ec-95e1-316f-16f05c16330e">VM.Managed.DAFUL.DOE Namespace</a><br />