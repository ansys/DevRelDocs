# Body.AccuracyForMassCalc Property 
 

Gets or sets the accuracy for [mass calculate].

**Namespace:**&nbsp;<a href="bdeeb71e-08d7-cd91-e11a-795ab9385ff7">VM.Managed.CAD</a><br />**Assembly:**&nbsp;VMAppCore (in VMAppCore.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
public double AccuracyForMassCalc { get; set; }
```

**VB**<br />
``` VB
Public Property AccuracyForMassCalc As Double
	Get
	Set
```

**C++**<br />
``` C++
public:
property double AccuracyForMassCalc {
	double get ();
	void set (double value);
}
```

**F#**<br />
``` F#
member AccuracyForMassCalc : float with get, set

```


#### Property Value
Type: Double<br />The accuracy for [mass calculate].

## See Also


#### Reference
<a href="dc98f941-e8ef-25f9-9a55-0151997e3c14">Body Class</a><br /><a href="bdeeb71e-08d7-cd91-e11a-795ab9385ff7">VM.Managed.CAD Namespace</a><br />