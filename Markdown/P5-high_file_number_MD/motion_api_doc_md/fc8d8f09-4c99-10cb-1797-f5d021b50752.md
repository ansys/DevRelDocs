# SubSystemDocument.TemplateXmlVersion Property 
 

Gets the version of template XML.

**Namespace:**&nbsp;<a href="8790c1b2-b85d-f406-6b72-1668fe47b24f">VM.Managed.DAFUL</a><br />**Assembly:**&nbsp;VMDFBase (in VMDFBase.dll) Version: 24.1.0.0

## Syntax

**C#**<br />
``` C#
public string TemplateXmlVersion { get; }
```

**VB**<br />
``` VB
Public ReadOnly Property TemplateXmlVersion As String
	Get
```

**C++**<br />
``` C++
public:
property String^ TemplateXmlVersion {
	String^ get ();
}
```

**F#**<br />
``` F#
member TemplateXmlVersion : string with get

```


#### Property Value
Type: String<br />The version of template XML.

## See Also


#### Reference
<a href="e9919a27-be97-b564-ea2b-cbc9db8ce58d">SubSystemDocument Class</a><br /><a href="8790c1b2-b85d-f406-6b72-1668fe47b24f">VM.Managed.DAFUL Namespace</a><br />