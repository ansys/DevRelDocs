# PrimitiveBase.GetSubPickerResultXml Method 
 

Gets the sub picker result XML.

**Namespace:**&nbsp;<a href="1c5ed3ad-7c83-7286-7fb9-90c4f0847ee4">VM.Managed.Picker</a><br />**Assembly:**&nbsp;VMPB (in VMPB.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
protected virtual string GetSubPickerResultXml()
```

**VB**<br />
``` VB
Protected Overridable Function GetSubPickerResultXml As String
```

**C++**<br />
``` C++
protected:
virtual String^ GetSubPickerResultXml()
```

**F#**<br />
``` F#
abstract GetSubPickerResultXml : unit -> string 
override GetSubPickerResultXml : unit -> string 
```


#### Return Value
Type: String<br />

## See Also


#### Reference
<a href="538ab7ce-4b1d-3ea3-3cbb-e9b962bafa21">PrimitiveBase Class</a><br /><a href="1c5ed3ad-7c83-7286-7fb9-90c4f0847ee4">VM.Managed.Picker Namespace</a><br />