# Settings.ANSYSRstReaderPath Property 
 

Gets the ANSYSRstReader.dll path.

**Namespace:**&nbsp;<a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre</a><br />**Assembly:**&nbsp;dfpredat (in dfpredat.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
public string ANSYSRstReaderPath { get; }
```

**VB**<br />
``` VB
Public ReadOnly Property ANSYSRstReaderPath As String
	Get
```

**C++**<br />
``` C++
public:
property String^ ANSYSRstReaderPath {
	String^ get ();
}
```

**F#**<br />
``` F#
member ANSYSRstReaderPath : string with get

```


#### Property Value
Type: String<br />The ANSYSRstReader.dll.

## See Also


#### Reference
<a href="67de487e-0228-b2ad-6f64-6d2b5653647d">Settings Class</a><br /><a href="d4810bcf-3d65-2bb9-b9ab-5575e456eab5">VM.Managed.DAFUL.Pre Namespace</a><br />