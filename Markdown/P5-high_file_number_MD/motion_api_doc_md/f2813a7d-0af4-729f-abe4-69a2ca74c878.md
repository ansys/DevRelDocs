# ObjectEventCore.OnRemoved Event
 

Occurs when [on remove].

**Namespace:**&nbsp;<a href="05a7f2f9-3551-60b1-3652-371a61dfb10b">VM.Managed</a><br />**Assembly:**&nbsp;VMAppCore (in VMAppCore.dll) Version: 24.1.23299.31577

## Syntax

**C#**<br />
``` C#
internal event EventHandler<RemoveFromDocEventArgs> OnRemoved
```

**VB**<br />
``` VB
Friend Event OnRemoved As EventHandler(Of RemoveFromDocEventArgs)
```

**C++**<br />
``` C++
internal:
virtual  event EventHandler<RemoveFromDocEventArgs^>^ OnRemoved {
	void add (EventHandler<RemoveFromDocEventArgs^>^ value);
	void remove (EventHandler<RemoveFromDocEventArgs^>^ value);
}
```

**F#**<br />
``` F#
internal abstract OnRemoved : IEvent<EventHandler<RemoveFromDocEventArgs>,
    RemoveFromDocEventArgs>
internal override OnRemoved : IEvent<EventHandler<RemoveFromDocEventArgs>,
    RemoveFromDocEventArgs>
```


#### Value
Type: System.EventHandler(<a href="a77ffb6c-2af6-24c5-cf79-8ae5a06304ff">RemoveFromDocEventArgs</a>)

#### Implements
<a href="98e3cfaa-c33a-1b1b-8497-8e613c08edf8">ILinkContainerEvent.OnRemoved</a><br />

## See Also


#### Reference
<a href="95892e40-8811-f191-46ab-8245b03d9b4c">ObjectEventCore Class</a><br /><a href="05a7f2f9-3551-60b1-3652-371a61dfb10b">VM.Managed Namespace</a><br />