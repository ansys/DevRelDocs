...
-   ![](images/img(6).jpg) / ![](images/img(5).jpg)/ ![](images/img(12).png): disable/enable the loop playback, an infinite number of times or a specified number of times. The Ansys Sound: Analysis and Specification application contains functionality for creating loopable samples without any clicks or audio spikes.
...
The second line defines the columns: Time\(s\), the three coordinate columns dependent on the selected mode, and the signal gain in dB, as described below.
...
-   A = Virtools/unity mode, in which:
    ...
    -   GaindB is the gain in dB applied to the sound at that point in the trajectory
...
The other lines are the sequence of **Time X Y Z GaindB** or **Time A E D GaindB** information.
