...
Since this operation is likely to be performed as a batch operation, see the notebook on
[Storing Credentials Securely](../streamlined/22_Storing_credentials_securely.ipynb) for more information how to
connect to Granta MI securely in a batch Python script.
...
one. Use a dictionary with the `record.history_identity` as the key to ensure you only get unique records (a single
...
for test_attribute in COMPARISON_ATTRIBUTES.values():
    ...
    unique_unprocessed_records.update({r.history_identity: r for r in unprocessed_records_for_attribute})
```
...
```
    ...
    MTS-615756
     - 0.02% Offset Yield Stress skipped: <Already populated>
     - 0.2% Offset Yield Stress skipped: <No data>
     - Young's Modulus (11-axis) skipped: <Already populated>
