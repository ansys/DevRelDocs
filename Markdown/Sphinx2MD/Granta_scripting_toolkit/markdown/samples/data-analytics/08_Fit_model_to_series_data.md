...
attributes plus the Pseudo-Attribute `Name` for efficiency. Filter out records that do not have values for the required attributes.
Store the record objects in a dictionary keyed by the
test temperature, and extract the unit symbols for use when displaying results.
...
for test_group in temperature_test_groups:
    ...
    fetcher = mpy.AttributeFetcher(test_data_table,
...
![Strain = Stress/Elastic Modulus + K\*(Stress/Elastic Modulus)\^n](./RambergOsgood.svg 'Ramberg-Osgood equation')
...
def get_response_data(records: List[mpy.Record]) -> np.ndarray:
    ...
    # The Granta MI Scripting Toolkit stores all functional data as ranges. Since both values are the same,
    # take the first.
...
```
    ...
    {'k': 0.1486349444618865,
     'modulus': 2062.020203463984,
     'n': 8.791864146426017,
     'yield_point': (1.2342202393692518, 2132.583028410755)}
    
    
    Fit parameters at Temperature 296.1K:
    {'k': 0.17913268159309942,
     'modulus': 1981.6619868475675,
     'n': 9.11459391609692,
     'yield_point': (1.2120542800540215, 2005.5494954094368)}
    
    
    Fit parameters at Temperature 422.0K:
    {'k': 0.306888602795139,
     'modulus': 1904.6229678299835,
     'n': 8.456684423703638,
     'yield_point': (1.1505510051329448, 1810.4412764700833)}
    
    
    Fit parameters at Temperature 588.7K:
    {'k': 0.4638673643943285,
     'modulus': 1825.4403077279012,
     'n': 8.486132367411276,
     'yield_point': (1.1055238444081892, 1652.979725191437)}
    
    
    Fit parameters at Temperature 699.8K:
    {'k': 0.478838023322276,
     'modulus': 1720.6433348881853,
     'n': 7.162987959853815,
     'yield_point': (1.0851718256655425, 1523.0650020622224)}
    
    
    Fit parameters at Temperature 810.9K:
    {'k': 3.0310551686407154,
     'modulus': 1501.671776404187,
     'n': 6.911013632913306,
     'yield_point': (0.874769543614486, 1013.2823792230079)}
