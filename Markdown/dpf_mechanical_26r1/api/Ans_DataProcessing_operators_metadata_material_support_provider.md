...
Reads the material support.   ///available inputs: streams_container (StreamsContainer) (optional), data_sources (DataSources)
            available outputs: abstract_field_support (AbstractFieldSupport)

material_support_provider()
material_support_provider(streams_container: object, data_sources: object, config: OperatorConfig)
material_support_provider(config: OperatorConfig)
...
```python
material_support_provider(streams_container, data_sources, config)
```
