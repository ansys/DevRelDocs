...
![Figure 1: Sequence of steps to execute mapping](ParticipantStepsInMapping.png)  
...
The only exception is when a) running in parallel using one of the
supported MPI versions and b) implementing the APIs on a subset
of all parallel processes.
In this case, start by calling `syscStartStandaloneParallel` function
in C or `syscStartStandaloneF` function in Fortran and by passing the
desired MPI communicator into those functions.
See
