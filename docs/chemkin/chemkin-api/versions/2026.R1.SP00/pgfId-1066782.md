...
the letters `CK` so that _Gas-phase Kinetics_ subroutines are easily
recognized and so that they are likely different from any user subroutine
names. The four remaining letters identify the purpose of the subroutine: The
first one or two usually refer to the variable that is being computed; the
last letters refer to either the input variables or the units.
...
of _Chemkin_. However, there are a few cases where either the functionality is
