...
subroutine. These options are described in more detail in [Chemkin Input Manual](https://ansyshelp.ansys.com/public/account/secured?returnurl=/Views/Secured/corp/v242/en/chemkin_in/chemkin_in.html). Briefly, to turn on user-programmed
rate routines, the user must include either `USRPROG`, or `USRPROD` auxiliary
keywords in the _Gas-phase Kinetics_ or _Surface Kinetics_ input file. The
