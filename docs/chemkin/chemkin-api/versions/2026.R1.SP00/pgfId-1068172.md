...
| --- | --- |
| `SUBROUTINE CKAVG (NN, S1, S2, SAVG)` | For arrays of length _nn_, SAVG(_n_) is the average value of S1(_n_) and S2(_n_). |
| `REAL FUNCTION CKBSEC (NPTS, X, XX, TT)` | Interpolate f(_X_) using bisection, given _X_ and other pairs of _X_ and f(_X_). |
...
| `SUBROUTINE CKSUBS (LINE, LOUT, NDIM, SUB, NFOUND, KERR)` | Returns an array of substrings in a character string with blanks or tabs as delimiters. |
