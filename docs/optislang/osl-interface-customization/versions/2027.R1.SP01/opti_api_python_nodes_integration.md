...
**Incoming Arguments**
- [Common Incoming Key Word Arguments (KWARGs)](#table-7-common-incoming-key-word-arguments-kwargs)
...
**Incoming Arguments**
- [Common Incoming Key Word Arguments (KWARGs)](#table-7-common-incoming-key-word-arguments-kwargs)
- [Additional Incoming KWARGs for the Execute Function](#table-8-additional-incoming-kwargs-for-the-execute-function)
...
**Incoming Arguments**
- [Common Incoming Key Word Arguments (KWARGs)](#table-7-common-incoming-key-word-arguments-kwargs)
- `input`: [HidSpecificInputDataIntegrations](#table-11-hidspecificinputdataintegrations-attributes) An aggregate of various input data.
...
- [HidSpecificInputDataIntegrations](#table-11-hidspecificinputdataintegrations-attributes): An aggregate of various input data which may differ for each Hierarchical ID (HID).
- [HidSpecificOutputDataIntegrations](#table-12-hidspecificoutputdataintegrations-attributes): An aggregate of various output data which may differ for each Hierarchical ID (HID).
- RunStatus: An object for documenting success and logging.
- [InputValueDefinition](#table-13-contents-of-inputvaluedefinition): An object for defining inputs.
...
|------|------|-------------|
| hid | HID | Hierarchical ID of a design variation, for example, `[0.99.4]`. |
| output_slot_values | Dictionary | A dictionary of name-value pairs that should be written to the output slots specified in the [configuration file](opti_api_python_nodes_config_files.md). |
...
In the Python code inside the `*_ci.py` file, you can import other libraries, allowing you to add a larger package of files to a complex integration plugin. With each additional `*_ci.py` file, you provide an additional node in optiSLang’s module library. For the sake of organizational clarity, modularity, reusability, and testability, it can be beneficial to keep only small portions of code in the `*_ci.py` files and concentrate the function and class definitions in a library of one or several files to import from. This way, you can create multiple nodes which build on a common modular library.
...
For more details, see [Configuration Files for Nodes](opti_api_python_nodes_config_files.md).
...
File name convention: `*_ci.svg`, `*_ci.ico`, `*_ci.png`, `*_ci.jpg`, `*_ci.jpeg`, `*_ci.bmp`
...
File name convention: `*_ci.qml`

### Description File
This is an optional file. You can use it to write a description of your plugin which is displayed in optiSLang's About Add-Ins dialog box. There is no file name convention, the file name is specified in the [configuration file](opti_api_python_nodes_config_files.md).

### Legal Note File
This is an optional file. You can use it to write a legal note of your plugin which is displayed in optiSLang's About Add-Ins dialog box. There is no file name convention, the file name is specified in the [configuration file](opti_api_python_nodes_config_files.md).
