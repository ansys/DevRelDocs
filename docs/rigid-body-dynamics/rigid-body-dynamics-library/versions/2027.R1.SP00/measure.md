...
Most useful measures pre-exist in the Rigid Dynamics model, and can be
...
computed at each time step (see the component measure example below).
...
A measure can use direct calculation or be time integrated. For a measure that
uses direct calculation, it is possible to retrieve the measure value through
time. For a measure that is time-integrated, both values and time derivatives
can be retrieved.
...
Returns a two-dimensional array. This function must be called after the
...
joint is 1. It contains the relative joint rotation velocity about the z axis
...
* For universal joints, this measure contains the two joint axis rotational velocities. (The first one about the X axis of the reference coordinate system and the second about the Z axis of the moving coordinate system.) These angles are expressed in radians.
...
This measure contains the total forces and moments that develop in the joint.
...
This measure allows tracking of the position of a Body Coordinate System
over time.
...
* Values 3 to 5 are the coordinates of the point where the interaction between the two bodies is reduced to a force; in other words, where the total torque is zero.

* Values 6 to 8 are the frictional forces between the two bodies.
