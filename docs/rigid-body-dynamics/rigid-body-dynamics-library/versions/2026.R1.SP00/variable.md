...
constant must be provided. More complex variables can be built using a
function variable. A function variable is a function of input, where the input is
given by a [measure](measure.md) and the function is
described by a table. In some cases, you can replace the table or the
...
measure. The input of the variable is formed by the values of the
...
joint condition using a function. Note that the literal variable is always called
