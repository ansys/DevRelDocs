...
object that allows access to all other underlying objects. This environment is
associated with an environment object in the Mechanical tree. Many
environments can exist in the same model. The model is known as the System in
the Rigid Dynamics Object model. This system contains the physical
representation of the model, and the environment contains the representation
of a given simulation performed on the model. This means that Bodies and
Joints belong to the systems, and Joint Conditions or Loads are available
on the environment.

You can access an object using its unique ID, which is the same ID used by
the Mechanical application. Global object tables help you to access an object
for which you have an ID.
...
Some objects must be created by calling the object constructor. For
