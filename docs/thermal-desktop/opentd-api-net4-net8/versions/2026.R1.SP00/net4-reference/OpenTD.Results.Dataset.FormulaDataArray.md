...
For formula syntax, see .NET docs for DataColumn.Expression.
...
the formula. This is necessary for a formula like "X0 + 30", for example,
when X0 is a temperature and 30 is in deg C.
Defaults to the WorkingUnits.

```csharp
public UnitsData FormulaUnits { get; set; }
```
...
1 if there are no input arrays. This value can be set by the user.
...
```csharp
public override DataSubtype Subtype { get; set; }
```
