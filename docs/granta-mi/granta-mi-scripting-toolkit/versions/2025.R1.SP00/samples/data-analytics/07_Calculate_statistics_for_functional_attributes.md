...
confidence_intervals = st.t.interval(0.95, grouped_by.count().T, loc=grouped_by.mean().T, scale=grouped_by.std(numeric_only=True).T)
```
...
for temperature_index, temperature in enumerate(grouped_by.groups):
    ...
    df_plot = pd.concat([grouped_by.mean().T[temperature].rename('mean'),
