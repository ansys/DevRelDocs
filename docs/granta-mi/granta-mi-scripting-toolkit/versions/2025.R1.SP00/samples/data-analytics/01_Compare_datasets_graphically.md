...
Create a DataFrame `build_df` that contains the build records. Set the DataFrame index to be the *Build ID* attribute value,
and sort by the index to guarantee a repeatable row order.
We use `.dropna()` to ensure that only fully populated records are kept in the dataframe.
...
```python
ax = sns.scatterplot(data=df_processed,
                     x="Travel Speed",
