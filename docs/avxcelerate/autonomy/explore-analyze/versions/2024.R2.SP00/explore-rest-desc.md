...
retrieve Logical Scenario and Algorithms data, Monitor the system, retrieve data from the executed simulation, create simulation reports, etc. 

This REST API is used to manage the data available in the Explore and analyze application. 
The REST API provides endpoints to retrieve Logical Scenario and Algorithms data, Monitor the system, retrieve data from the executed simulation and create simulation reports. 
...
You require a token to use the REST API interface.
...
Copy and save the generated token so that you use it to authenticate.
The mandatory attributes are `refresh_token` and `access_token`. 

Access REST API through Swagger with Token:

1. Make sure to have generated the token before trying to access the REST API. 
2. Generate Access token (workflow above).
3. Go the REST API documentation page.
4. Click **Authorize**.
