...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| formats | [vss.CameraDataFormat](#cameradataformat) | repeated | The types of data produced by the camera. <br>Can be the following: `RAW`, `TEMPERATURE_MAP`, `SPECTRAL_IRRADIANCE_MAP` or `CUSTOM_DATA`. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| formats | [vss.LidarDataFormat](#lidardataformat) | repeated | The format(s) of the data produced by a lidar sensor. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| formats | [vss.RadarDataFormat](#radardataformat) | repeated | The format(s) of the data produced by a radar sensor. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| sensor_id | [vss.ResourceIdentifier](#resourceidentifier) |  | The identifier of the sensor which produced the data.<br>Note: The identifier corresponds to the identifier defined in the sensor configuration. |
| data_id | [SensorDataIdentifier](#sensordataidentifier) |  | **Deprecated.** The identifier of the produced data.<br>To be used by the subscriber to retrieve the sensor data.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `data_by_identifiers` instead (see `SensorDataInfo.data_id`). |
| simulation_time | google.protobuf.Duration |  | The simulation time of the produced data. |
| metadata | [SensorMetadata](#sensormetadata) |  | **Deprecated.** SensorMetadata.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `data_by_identifiers` instead (see `SensorDataInfo.metadata`). |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| camera_metadata | [CameraMetadata](#camerametadata) |  | Metadata for data produced by a camera sensor. |
| lidar_metadata | [LidarMetadata](#lidarmetadata) |  | Metadata for data produced by a lidar sensor. |
| radar_metadata | [RadarMetadata](#radarmetadata) |  | Metadata for data produced by a radar sensor. |
| radar_debug_view_metadata | [RadarDebugViewMetadata](#radardebugviewmetadata) |  | Metadata for a debug view image produced by a radar sensor. |
| camera_feedback | [CameraFeedback](#camerafeedback) |  | Current values of some of the camera sensor's parameters that were used to produce this data. |
...
| Method Name | Request Type | Response Type | Description |
| ----------- | ------------ | ------------- | ------------|
| Send | [FeedbackControlDescription](#feedbackcontroldescription) | [.vss.Status](#status) | Sends a sensor (camera, lidar, radar) feedback control.<br>Returns a processing Status. |
...
| demosaicing | [Demosaicing](#demosaicing) |  | Demosaicing output. |
| injection | [Injection](#injection) |  | Injection output. |
...
| thermal_noise_simple | [ThermalNoiseSimpleModel](#thermalnoisesimplemodel) |  | ThermalNoiseSimpleModel. |
| thermal_noise_advanced | [ThermalNoiseAdvancedModel](#thermalnoiseadvancedmodel) |  | ThermalNoiseAdvancedModel. |
...
| focal_length | google.protobuf.DoubleValue |  | **Deprecated.** Focal length of the lens.<br>Unit: meters (m)<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using one of the two possibilities of `lens_chromatic_aberration` instead: `SimpleChromaticAberration` or `AdvancedChromaticAberration`. |
...
| simple_chromatic_aberration | [SimpleChromaticAberration](#simplechromaticaberration) |  | Simple chromatic aberration. |
| advanced_chromatic_aberration | [AdvancedChromaticAberration](#advancedchromaticaberration) |  | Advanced chromatic aberration. |
...
| feedback_control_camera_parameters | [FeedbackControlCameraParameters](#feedbackcontrolcameraparameters) |  | This message is used to update a camera parameters during the simulation. |
| feedback_control_radar_parameters | [FeedbackControlRadarParameters](#feedbackcontrolradarparameters) |  | This message is used to update a radar parameters during the simulation. |
| feedback_control_lidar_parameters | [FeedbackControlLidarParameters](#feedbackcontrollidarparameters) |  | This message is used to update a lidar parameters during the simulation. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| rotating_lidar | [RotatingLidar](#rotatinglidar) |  | RotatingLidar. |
| flash_lidar | [FlashLidar](#flashlidar) |  | FlashLidar. |
...
| position | [vss.Vector3D](#vector3d) |  | Position of the antenna relative to the radar device's position.<br>Unit: meters (m) |
| orientation | [vss.EulerAngles](#eulerangles) |  | Orientation of the antenna relative to the radar device's orientation.<br>Unit: radians (rad) |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| system_frequency_modulated_continuous_waveform | [SystemFrequencyModulatedContinuousWaveform](#systemfrequencymodulatedcontinuouswaveform) |  | SystemFrequencyModulatedContinuousWaveform. |
| performance_frequency_modulated_continuous_waveform | [PerformanceFrequencyModulatedContinuousWaveform](#performancefrequencymodulatedcontinuouswaveform) |  | PerformanceFrequencyModulatedContinuousWaveform. |
| arbitrary_system_frequency_modulated_continuous_waveform | [ArbitrarySystemFrequencyModulatedContinuousWaveform](#arbitrarysystemfrequencymodulatedcontinuouswaveform) |  | ArbitraryFrequencyModulatedContinuousWaveform. |
...
| pedestal_parametric_phase_noise | [PedestalParametricPhaseNoise](#pedestalparametricphasenoise) |  | Phase noise defined as a pedestal parametric model. |
| piecewise_linear_phase_noise | [PiecewiseLinearPhaseNoise](#piecewiselinearphasenoise) |  | Phase noise defined as a piecewise linear model, using a sequence of power spectral densities. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| system_pulse_doppler_waveform | [SystemPulseDopplerWaveform](#systempulsedopplerwaveform) |  | SystemPulseDopplerWaveform. |
| performance_pulse_doppler_waveform | [PerformancePulseDopplerWaveform](#performancepulsedopplerwaveform) |  | PerformancePulseDopplerWaveform. |
| arbitrary_system_pulse_doppler_waveform | [ArbitrarySystemPulseDopplerWaveform](#arbitrarysystempulsedopplerwaveform) |  | ArbitrarySystemPulseDopplerWaveform |
...
| roll_off_order | int32 |  | The roll-off order, associated to the Order type.<br>Range: [1, 50] |
| roll_off_point | [RollOffPoint](#rolloffpoint) |  | The roll-off definition, associated to the Set Point type. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| weightings_by_transmitter | [WeightingByTransmitter](#weightingbytransmitter) |  | WeightingByTransmitter. |
| weightings_by_pulses_or_chirps | [WeightingByPulsesOrChirps](#weightingbypulsesorchirps) |  | WeightingByPulsesOrChirps. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| pulse_doppler_waveform | [PulseDopplerWaveform](#pulsedopplerwaveform) |  | PulseDopplerWaveform. |
| frequency_modulated_continuous_waveform | [FrequencyModulatedContinuousWaveform](#frequencymodulatedcontinuouswaveform) |  | FrequencyModulatedContinuousWaveform. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| flat_window | [FlatWindow](#flatwindow) |  | FlatWindow. |
| hamming_window | [HammingWindow](#hammingwindow) |  | HammingWindow. |
| hann_window | [HannWindow](#hannwindow) |  | HannWindow. |
| taylor_window | [TaylorWindow](#taylorwindow) |  | TaylorWindow. |
...
| Method Name | Request Type | Response Type | Description |
| ----------- | ------------ | ------------- | ------------|
| Switch | [.vss.ObjectIdentifier](#objectidentifier) | [.vss.Status](#status) | Changes the active Lighting System to the one defined as input parameter. |
...
| flux | double | optional | Flux of the lamp.<br>Unit: lumens (lm) |
| light_functions | string | repeated | **Deprecated.** Light function tags.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `light_functions_tags` instead. |
...
| position | [vss.Vector3D](#vector3d) |  | Position of the module relative to the projector.<br>Unit: meters (m) |
| orientation | [vss.EulerAngles](#eulerangles) |  | Orientation of the module relative to the projector.<br>Unit: degrees |
...
| hmin | float | optional | Minimal horizontal angle corresponding to the bottom border of the pixel beam area.<br>Unit: degrees<br>Range: [-180, `hmax`[ |
| hmax | float | optional | Maximal horizontal angle corresponding to the top border of the pixel beam area.<br>Unit: degrees<br>Range: ]`hmin`, 180] |
| vmin | float | optional | Minimal vertical angle corresponding to the left border of the pixel beam area.<br>Unit: degrees<br>Range: [-90, `vmax`[ |
| vmax | float | optional | Maximal vertical angle corresponding to the right border of the pixel beam area.<br>Unit: degrees<br>Range: ]`vmin`, 90] |
| width | int32 | optional | Horizontal resolution expressed in pixels.<br>Range: ]0, 3840] |
| height | int32 | optional | Vertical resolution expressed in pixels.<br>Range: ]0, 2160] |
...
| position | [vss.Vector3D](#vector3d) |  | Position of the projector relative to the Lighting System origin.<br>Unit: meters (m) |
| orientation | [vss.EulerAngles](#eulerangles) |  | Orientation of the projector relative to the Lighting System origin.<br>Unit: degrees |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| isobarycenter | [vss.Vector3D](#vector3d) |  | Isobarycenter coordinates in the camera reference frame.<br>Unit: meters (m) |
| dimensions | [vss.Vector3D](#vector3d) |  | Dimension of the object in the world reference frame.<br>Unit: meters (m) |
| orientation | [vss.EulerAngles](#eulerangles) |  | Orientation of the object in the camera reference frame.<br>Unit: radians (rad) |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| camera_format | [vss.CameraDataFormat](#cameradataformat) |  | Type of image produced by the camera.<br>Can be one of the following: `RAW`, `TEMPERATURE_MAP`, `SPECTRAL_IRRADIANCE_MAP` or `CUSTOM_DATA`. |
...
| bounding_box_3d | [BoundingBox3D](#boundingbox3d) |  | 3D Bounding box produced. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| point_cloud_data | [PointCloudData](#pointclouddata) |  | Point cloud data produced by the lidar sensor. |
| waveform_data | [WaveformData](#waveformdata) |  | Digital waveform data produced by the lidar sensor. |
| contribution_data | [ContributionData](#contributiondata) |  | Contribution List produced for the lidar sensor. |
| lens_output_data | [LensOutputData](#lensoutputdata) |  | Lens Output data produced by the lidar sensor. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| radar_data | [RadarOutputData](#radaroutputdata) |  | RadarOutputData |
...
| camera_data | [CameraData](#cameradata) |  | The type of data produced by the camera sensor. |
| lidar_data | [LidarData](#lidardata) |  | The type of data produced by the lidar sensor. |
| radar_data | [RadarData](#radardata) |  | The type of data produced by the radar sensor. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| vss_identity | [vss.ObjectIdentifier](#objectidentifier) |  | The unique identifier of the asset.<br>Note: The identifier is case-sensitive. |
...
| deploy_parameters | [EndPointRange](#endpointrange) |  | **Deprecated.** The infrastructure deploy parameters.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `DeployConfiguration` instead. |
...
| lighting_system | [LightingSystem](#lightingsystem) |  | **Deprecated.** The Lighting System configuration of the ego vehicle.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `LightingSystemConfiguration` instead. |
| ego_vehicle_identity | [vss.ObjectIdentifier](#objectidentifier) |  | The unique identifier of the ego vehicle, which is the vehicle instance to which the sensor configuration and Lighting System(s) are attached. |
...
| lighting_system_data | bytes |  | The encapsulated byte array. |
...
| active_lighting_system | [vss.ObjectIdentifier](#objectidentifier) |  | The Lighting System that is set as active.<br>Note: If this value is not set, the first Lighting System associated with the ego vehicle is set as active. |
...
Configuration parameters for AVX deployment.

Warning: This message is marked as deprecated and might be removed in later versions. Consider using the `DeployConfiguration` message instead.

| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| host | string |  | **Deprecated.** Host used to spawn the AVX processes.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `DeployConfiguration.local_nodes` instead. |
| min_port | int32 |  | **Deprecated.** The first TCP port used to communicate.<br>AVX allocates ports to communicate with different processes starting from this min_port value.<br>Note: The value must be greater than 1024.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `DeployConfiguration.local_nodes` instead. |
| max_port | int32 |  | **Deprecated.** The last TCP port used to communicate.<br>AVX allocates ports to communicate with different processes up to this last port.<br>Note: The value must be greater than the min_port value.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `DeployConfiguration.local_nodes` instead. |
...
| index | string |  | The index number of the GPU.<br>Note: The GPU index might not be consistent between PC reboots. <br>Therefore, it is recommended to use the PCI bus ID (`bus_id`) or the <br>UUID (`uuid`) instead. <br>For more details, refer to AVxcelerate Sensors Simulator User's Guide. |
| bus_id | string |  | The PCI bus identifier of the GPU.<br>Note: The PCI bus identifier must have the format a:b:c.d with a = domain ID, b = bus ID, c = device ID, d = function ID and each represented as base-16 number with 1 to 8 digits. |
| uuid | string |  | The UUID of the GPU.<br>Note: The UUID must be a string starting with an upper-case 'GPU' <br>followed by five base-16 number blocks separated with a hyphen. The blocks must have the following amount of digits: 8, 4, 4, 4 and 12. <br>Example: GPU-12345678-1234-1234-1234-123456789abc |
...
| Method Name | Request Type | Response Type | Description |
| ----------- | ------------ | ------------- | ------------|
| Load | [Configuration](#configuration) | [.vss.Status](#status) | Loads AVX with a track, a list of assets, some simulation parameters, a sensor configuration, a Lighting System (optional), some deployment parameters and the ID of the ego vehicle (the vehicle instance to which the sensor configuration will be attached). |
| Initialize | [WorldUpdate](#worldupdate) | [.vss.Status](#status) | Sets the desired initial position, orientation and state of all the components of the world. Changes the AVX state to RUNNING.<br>Note: When called after the Stop command, the Initialize command resets the sensors and advanced lighting systems to their initial loaded states. <br>This simulation reset is a Beta feature in the current release. For more information on this feature, refer to the Simulation section of the <br>API description chapter in the API documentation. |
| Update | [WorldUpdate](#worldupdate) | [.vss.Status](#status) | Applies a world model update to AVX. |
| Stop | .google.protobuf.Empty | [.vss.Status](#status) | Stops the simulation. Changes the AVX state from RUNNING to LOADED. |
| Unload | .google.protobuf.Empty | [.vss.Status](#status) | Unloads the simulation. Changes the AVX state back to STARTED.<br>Note: This command allows the restart of a simulation without having to restart AVX. |
| Kill | .google.protobuf.Empty | [.vss.Status](#status) | Sends a signal to end all the AVX processes properly. <br>Closes the gRPC server. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| recording_format | [RecordingFormat](#recordingformat) |  | Activation of output data saving to disk, and configuration of the type of data and file format.<br>Refer to the `RecordingFormat` message for more information.<br>Note: Optional field. If not set, sensor data is not written to disk. |
| shared_memory_access | google.protobuf.BoolValue |  | Activation of output data production in shared memory.<br>Default value: true<br>Note: Optional field. If set to `true` or if not set, storage in shared memory is activated, and data can be accessed via a notification-based mechanism. |
| remote_direct_memory_access | [RemoteDirectMemoryAccess](#remotedirectmemoryaccess) |  | Activation and configuration of output data production for <br>Remote Direct Memory Access using the pre-build RDMA sender.<br>Note: This parameter only applies to camera output data. <br>Note: Optional field. If not set, no sensor data is generated for RDMA.<br>Note: Data access using Remote Direct Memory Access (RDMA) technology is delivered as a beta feature in the current release. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| cartesian_grid | [CartesianGrid](#cartesiangrid) |  | The Cartesian grid.<br>Note: Cartesian grid applies to flashing lidar only. |
| polar_grid | [PolarGrid](#polargrid) |  | The polar grid.<br>Note: Polar grid applies to rotating lidar only. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| camera_near_distance | double | optional | Distance at which the camera starts seeing objects.<br>Unit: meters (m) <br>Range: ]0.0, `camera_culling_distance`] or ]0.0, inf[ if `camera_culling_distance` == 0<br>Default value: 0.3 |
| camera_culling_distance | double | optional | Distance at which the camera stops seeing objects.<br>Unit : meters (m)<br>Range: [`camera_near_distance`, inf[ or 0<br>Note: Setting the value to 0 means infinite.<br>Default value: 1000 |
| screen_space_ambient_occlusion | [ScreenSpaceAmbientOcclusion](#screenspaceambientocclusion) |  | Screen-Space Ambient Occlusion. |
| occlusion_culling | bool | optional | Activation of the Occlusion Culling experimental feature.<br>Note: It improves performances, but can lead to unexpected behaviors.<br>Default value: true |
| raytraced_shadows | bool | optional | Activation of the Raytraced Shadows experimental feature.<br>Note: It improves the quality of the shadows, but can lead to unexpected behaviors.<br>Default value: true |
| shadow_quality | [Qualities](#qualities) |  | The camera shadow quality.<br>Default value: MEDIUM |
| antialiasing_factor | double | optional | The camera antialiasing factor.<br>Range: [1.0, 4.0]<br>Default value: 1.0 |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| sampling_rate | double |  | Angular sampling used when computing the rendering of the light sources in the Lighting System.<br>Unit: degrees<br>Range: [0.05, 57]<br>Default value: 0.05 |

### ManualBatching

Manual batching configuration.

Note: With the manual batching method, the number of batches for rays and rx channels must be specified.

| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| number_of_ray_batches | int32 |  | The number of batches on casted rays: applies to all the radar modes. |
...
| camera_ground_truth_parameters | [CameraGroundTruthParameters](#cameragroundtruthparameters) |  | **Deprecated.** Activation of ground truth output production that can be used to test or train a perception algorithm.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `RenderingParameters.RealTimeParameters.camera_ground_truth_parameters` instead. |
...
| ray_density | double |  | **Deprecated.** The linear ray density per radar wavelength (at max distance of 100 m).<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `grid_sampling` instead. |
| grid_sampling | [RadarGridSampling](#radargridsampling) |  | The sampling of the grid. |
| manual_batching | [ManualBatching](#manualbatching) |  | Configuration of the manual batching.<br>Note: Optional field. By default, the sensor execution is not batched. |
| automatic_batching | [AutomaticBatching](#automaticbatching) |  | Configuration of the automatic batching.<br>Note: Optional field. By default, the sensor execution is not batched. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| camera_recording_format | [vss.CameraDataFormat](#cameradataformat) |  | Format for saving camera output on disk.<br>Possible value:<br> - for physics-based camera: RAW, BMP, GIF, JPEG, or PNG,<br> - for thermal camera: TEMPERATURE_MAP. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| camera_near_plane | google.protobuf.FloatValue |  | **Deprecated.** The camera near plane value.<br>Unit: meters (m)<br>Range: ]0, `camera_far_plane`[<br>Default value: 0.1<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `RealTimeParameters.camera_near_plane` instead. |
| camera_far_plane | google.protobuf.FloatValue |  | **Deprecated.** The camera far plane.<br>Unit: meters (m)<br>Range: ]`CameraNearPlane`, float max]<br>Default value: 10000.0<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `RealTimeParameters.camera_far_plane` instead. |
| shadow_quality | [Qualities](#qualities) |  | **Deprecated.** The camera shadow quality.<br>Default value: EXTREME<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `RealTimeParameters.shadow_quality` instead. |
| texture_quality | [Qualities](#qualities) |  | **Deprecated.** The camera texture quality.<br>Default value: EXTREME<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `RealTimeParameters.texture_quality` instead. |
...
| antialiasing_factor | google.protobuf.FloatValue |  | **Deprecated.** The camera antialiasing factor.<br>Range: [1.0, 4.0]<br>Default value: 1.0<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `RealTimeParameters.antialiasing_factor` instead. |
...
| lpe_parameters | [LightPropagationEngineSimulationParameters](#lightpropagationenginesimulationparameters) |  | Camera simulation parameters for the Light Propagation Engine.<br>Note: The Light Propagation Engine is delivered as a beta feature in the current release. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| ssao_activation | bool |  | Toggle the screen space ambient occlusion feature.<br>Default value: false |
| ambient_occlusion_amount | double | optional | Linear gain factor of neighbouring occluders.<br>Range: [ 1.0, 8.0 ]<br>Default value: 1.0 |
| ambient_occlusion_radius | double | optional | Maximal radius to detect occluders.<br>Unit: meters (m)<br>Range: [0.01, 2.0]<br>Default: 0.8 |
| ambient_occlusion_power_exponent | double | optional | Amplification factor of the occlusion ambient lighting.<br>Range: [0.0, 8.0]<br>Default: 2 |
| ambient_occlusion_surface_bias | double | optional | Distance bias to avoid self shadowing.<br>Unit: meters (m)<br>Range: [0.0, 1.0]<br>Default value: 0.01 |
...
| recording_format | [RecordingFormat](#recordingformat) |  | **Deprecated.** Field indicating whether or not the sensor's outputs are saved as files dumped to the disk.<br>Note: Optional field. If not set, the sensor's outputs are not saved as file dumped to the disk.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using `DataAccess` instead. |
...
| radar_simulation | [RadarSimulation](#radarsimulation) |  | Specific parameters for a radar sensor. |
| lidar_simulation | [LidarSimulation](#lidarsimulation) |  | Specific parameters for a lidar sensor. |
| pb_cam_simulation | [PbCameraSimulation](#pbcamerasimulation) |  | Specific parameters for a physics-based camera sensor. |
| thermal_cam_simulation | [ThermalCameraSimulation](#thermalcamerasimulation) |  | Specific parameters for a thermal camera sensor. |
...
| pixel_segmentation_mapping | [vss.PixelSegmentationMapping](#pixelsegmentationmapping) |  | Configuration of the tag-color mapping used for the pixel segmentation camera ground truth data. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| resource_identifier | string |  | **Deprecated.** The identifier of the resource to upload.<br>Warning: This field is marked as deprecated and might be removed in later versions. Consider using the `values` field instead. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| date_time | google.protobuf.Timestamp |  | Time of day (as sent by the simulation framework) to apply to the AVX sky model. |
| streetLightsState | [State](#state) |  | State to apply to all the nodes of the track that hold the "StreetLight" tag.<br>Note: "ON" and "OFF" are the only accepted values. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| fog_activation | bool | optional | Activation of the fog in the simulation.<br>Default value: false |
| maximum_visibility_distance | double | optional | The maximum visibility is defined with respect to the Annex 3 to the <br>Convention on International Civil Aviation, i.e. a residual 5% of contrast will be obtained at the given maximum visibility distance.<br>Unit: meters (m)<br>Range: Strictly positive<br>Default value: 100 |
| color | [vss.Color](#color) |  | Color of the fog, defined in the RGB format.<br>Default value: [255, 255, 255] |
| spectrum | [vss.Spectrum](#spectrum) |  | Spectrum of the fog. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| position | [vss.Vector3D](#vector3d) |  | Position of the object in the world reference frame.<br>Unit: meters (m) |
| velocity | [vss.Vector3D](#vector3d) |  | Velocity of the object relative to the world reference frame.<br>Unit: meters per second (m/s) |
| orientation | [vss.EulerAngles](#eulerangles) |  | Orientation of the object relative to the world reference frame.<br>Note: When applying the Euler angles update, it is done in the standard order yaw, pitch, roll.<br>Unit: radians (rad) |
| angular_velocity | [vss.Vector3D](#vector3d) |  | The new angular velocity vector of the object expressed in the world reference frame. <br>Note: The value on the Y axis corresponds to the yaw rotation. The value on the X axis corresponds to the pitch rotation. The value on the Z axis corresponds to the roll rotation.<br>Unit: radians per second (rad/s) |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| vss_identity | [vss.ObjectIdentifier](#objectidentifier) |  | The unique identifier of an object in the simulation.<br>Note: The identifier is case-sensitive and must correspond to one defined in the loaded configuration. Each object has a unique `vss_identity`. |
...
| index | google.protobuf.Int32Value |  | The index.<br>Note: The `index` field is required with the "Wheel" and "TurnIndicator" tag when you need to identify one specific node of the object among several nodes holding the same tag. |
