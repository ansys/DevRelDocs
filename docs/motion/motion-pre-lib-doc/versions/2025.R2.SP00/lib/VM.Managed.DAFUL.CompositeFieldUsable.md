...
```csharp
public abstract class CompositeFieldUsable : CompositeField, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
protected CompositeFieldUsable(Property owner)
```
