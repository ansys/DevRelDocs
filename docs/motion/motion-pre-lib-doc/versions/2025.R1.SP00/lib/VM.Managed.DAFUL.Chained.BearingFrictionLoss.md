...
```csharp
public class BearingFrictionLoss : LinkContainer, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public BearingFrictionLoss(IOwned owner)
```
...
```csharp
public IConnector Bearing { get; set; }
```
