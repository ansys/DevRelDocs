...
```csharp
public abstract class MultiObjectContactGeometryFinderBase : LinkContainer, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public MultiObjectContactGeometryFinderBase(IOwned owner)
```
...
```csharp
public abstract ObjectBase GetContactGeometry(ObjectBase ob)
```
