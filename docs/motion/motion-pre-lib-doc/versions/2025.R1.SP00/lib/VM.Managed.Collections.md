#  Namespace VM.Managed.Collections

### Classes

 [DictionaryCollectionEnumerator<T, TValue, TLink, TValueWrapper, TValueConverter\>](VM.Managed.Collections.DictionaryCollectionEnumerator\-5.md)

 [LinkCollection<T, TLink, TCollection\>](VM.Managed.Collections.LinkCollection\-3.md)

 [LinkCollectionWithFixedAddition<T, TLink, TCollection\>](VM.Managed.Collections.LinkCollectionWithFixedAddition\-3.md)

 [LinkDictionaryCollection<T, TValue\>](VM.Managed.Collections.LinkDictionaryCollection\-2.md)

 [LinkDictionaryCollectionBase<T, TValue, TValueWrapper, TValueConverter\>](VM.Managed.Collections.LinkDictionaryCollectionBase\-4.md)

 [LinkLinkDictionaryCollection<T, TValue\>](VM.Managed.Collections.LinkLinkDictionaryCollection\-2.md)

 [LinkLinkedList<T, TLink\>](VM.Managed.Collections.LinkLinkedList\-2.md)

 [LinkListCollection<T, TLink\>](VM.Managed.Collections.LinkListCollection\-2.md)

 [LinkListCollectionWithFixedAddition<T, TLink\>](VM.Managed.Collections.LinkListCollectionWithFixedAddition\-2.md)

 [NamedObjectDictionary<T\>](VM.Managed.Collections.NamedObjectDictionary\-1.md)

This class is to represent dictionary of [named object].

### Structs

 [LinkDictionaryValueLinkConverter<TValue\>](VM.Managed.Collections.LinkDictionaryValueLinkConverter\-1.md)

 [LinkDictionaryValueSimpleConverter<TValue\>](VM.Managed.Collections.LinkDictionaryValueSimpleConverter\-1.md)

### Interfaces

 [IHasLink](VM.Managed.Collections.IHasLink.md)

 [ILinkCollection](VM.Managed.Collections.ILinkCollection.md)

 [ILinkDictionaryValueConverter<TValue, TValueWrapper\>](VM.Managed.Collections.ILinkDictionaryValueConverter\-2.md)

### Enums

 [ModifiedType](VM.Managed.Collections.ModifiedType.md)

