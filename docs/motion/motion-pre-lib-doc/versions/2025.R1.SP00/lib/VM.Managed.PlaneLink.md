...
```csharp
public class PlaneLink : PlaneBase, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable, IReservable
```
...
```csharp
public PlaneLink(ITransform pTransform, Plane planeType)
```
...
```csharp
public PlaneLink(ITransform pTransform)
```
