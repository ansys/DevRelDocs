...
```csharp
public class AttributeObjectFeature : AttributeBase, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public AttributeObjectFeature(bool bCanDelete)
```

#### Parameters

`bCanDelete` [bool](https://learn.microsoft.com/dotnet/api/system.boolean)
