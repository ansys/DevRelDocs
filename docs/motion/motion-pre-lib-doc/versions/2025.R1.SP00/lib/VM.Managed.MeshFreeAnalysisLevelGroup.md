...
```csharp
[Obsolete]
public class MeshFreeAnalysisLevelGroup : LinkContainer, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public MeshFreeAnalysisLevelGroup(uint levelCount)
```
