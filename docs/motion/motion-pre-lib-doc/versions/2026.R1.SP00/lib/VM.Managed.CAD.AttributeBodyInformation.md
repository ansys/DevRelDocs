...
<a id="VM_Managed_CAD_AttributeBodyInformation"></a>
...
```csharp
public class AttributeBodyInformation : AttributeBase, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public AttributeBodyInformation(int nIndex)
```
