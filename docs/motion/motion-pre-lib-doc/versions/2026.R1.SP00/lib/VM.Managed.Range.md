...
```csharp
public class Range : LinkContainer, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public Range(TransformBase transform, Variable radius)
```
...
```csharp
public Range(PointBase point, Variable radius)
```
