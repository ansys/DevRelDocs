# Namespace VM.Models.FE.Mesh
<a id="VM_Models_FE_Mesh"></a>

### Namespaces

 [VM.Models.FE.Mesh.Elements](VM.Models.FE.Mesh.Elements.md)

 [VM.Models.FE.Mesh.Generator](VM.Models.FE.Mesh.Generator.md)

 [VM.Models.FE.Mesh.Materials](VM.Models.FE.Mesh.Materials.md)

 [VM.Models.FE.Mesh.Properties](VM.Models.FE.Mesh.Properties.md)

 [VM.Models.FE.Mesh.Sets](VM.Models.FE.Mesh.Sets.md)

 [VM.Models.FE.Mesh.Writer](VM.Models.FE.Mesh.Writer.md)

### Classes

 [Entity](VM.Models.FE.Mesh.Entity.md)

 [HasMultipleNodes](VM.Models.FE.Mesh.HasMultipleNodes.md)

 [MeshFile](VM.Models.FE.Mesh.MeshFile.md)

 [Node](VM.Models.FE.Mesh.Node.md)

### Interfaces

 [IEntity](VM.Models.FE.Mesh.IEntity.md)

 [IPropertyBeam](VM.Models.FE.Mesh.IPropertyBeam.md)

 [ISet](VM.Models.FE.Mesh.ISet.md)

### Enums

 [BeamCrossSectionShapes](VM.Models.FE.Mesh.BeamCrossSectionShapes.md)

 [ElementTypes](VM.Models.FE.Mesh.ElementTypes.md)

