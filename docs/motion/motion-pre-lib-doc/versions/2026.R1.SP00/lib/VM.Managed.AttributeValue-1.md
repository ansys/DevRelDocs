...
<a id="VM_Managed_AttributeValue_1"></a>
...
```csharp
public class AttributeValue<TValue> : AttributeBase, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public AttributeValue(TValue value)
```
