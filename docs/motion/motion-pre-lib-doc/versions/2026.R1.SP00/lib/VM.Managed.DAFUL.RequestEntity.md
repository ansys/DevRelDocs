# Class RequestEntity
<a id="VM_Managed_DAFUL_RequestEntity"></a>
...
```csharp
public class RequestEntity : RequestBase, IObservableObject, IDisposableObject, ILinkable, ILinkContainerEvent, IVerifiable, IHistoryObjectSerializable, IAttributeContainer, ISubEntity, IEntityBase, IObjectBase, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, IHasKeyObject, IHasName, IEnabled, IHasComment, INamed, INavigatorItem, IMultiChangable, IHasReplaceableEntity, IRequestEntity
```
...
```csharp
public RequestEntity(string strName)
```
