...
<a id="VM_Managed_DirectionBaseAxis"></a>
...
```csharp
public class DirectionBaseAxis : DirectionBase, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable, IHasReplaceableEntity
```
...
```csharp
public DirectionBaseAxis(Coordinate AxisType)
```
