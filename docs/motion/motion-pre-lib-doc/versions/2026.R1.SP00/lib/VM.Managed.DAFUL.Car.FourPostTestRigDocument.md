...
<a id="VM_Managed_DAFUL_Car_FourPostTestRigDocument"></a>
...
```csharp
public FourPostTestRigDocument(UIntPtr docCore)
```
...
```csharp
public AnalysisBase GetAnalysisFromSimulationConfigurations(FourPostAnalysisType type)
```

#### Parameters

`type` [FourPostAnalysisType](VM.Managed.DAFUL.Car.FourPostAnalysisType.md)
...
```csharp
public FourPostAnalysisConfigurationBase GetFourPostConfigurationFromSimulationConfigurations(FourPostAnalysisType type)
```
