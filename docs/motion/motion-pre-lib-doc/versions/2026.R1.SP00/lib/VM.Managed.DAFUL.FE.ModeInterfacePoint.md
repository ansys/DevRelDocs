...
<a id="VM_Managed_DAFUL_FE_ModeInterfacePoint"></a>
...
```csharp
public class ModeInterfacePoint : FESubEntity, IObservableObject, IDisposableObject, ILinkable, ILinkContainerEvent, IVerifiable, IHistoryObjectSerializable, IAttributeContainer, ISubEntity, IEntityBase, IObjectBase, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, IHasKeyObject, IHasName, IEnabled, IHasComment, INamed, INavigatorItem, IFEObject, IDraw, IDrawIcon, IVisible, IReferencable
```
...
```csharp
public ModeInterfacePoint(string strName, Vector vecPosition)
```
...
The name of [mode interface point].
