...
You can also [configure a DPF installation](/docs/dpf/dpf-framework/getting-started/configuration) to mark explicit acceptance for every session.
...
To change this behavior, see [here](/docs/dpf/dpf-framework/getting-started/licensing).

To know if operators require a license increment checkout to run, check their **license**
attribute in the [Operator specifications reference](/docs/dpf/dpf-framework/operator-specifications/operator-specifications).

To check which Ansys licensing increments correspond to **any_dpf_supported_increments**,
see [Compatible Ansys license increments](/docs/dpf/dpf-framework/getting-started/licensing).
...
[server context](/docs/dpf/dpf-framework/user-guide/server-context)
...
- Starting a standalone DPF Server may fail due to the
  **DPF Preview License Agreement** (see [here](/docs/dpf/dpf-framework/getting-started/licensing)).
- Creating an operator may fail if the operator performs data transformation and no license server
  or license file is found (see [here](/docs/dpf/dpf-framework/getting-started/licensing)).
- Running an operator requiring a license checkout may fail if no
  [compatible license increment](/docs/dpf/dpf-framework/getting-started/licensing) is available or if the DPF Server context is **Entry**, 
  preventing any license check-out (see [here](/docs/dpf/dpf-framework/getting-started/licensing)).
