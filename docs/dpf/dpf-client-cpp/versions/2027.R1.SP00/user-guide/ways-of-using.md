...
These capabilities are provided through libraries of operators.
To learn more about the computed data and the operator concepts, see [Concepts and Terminology](concepts.md).

A DPF application is always composed of a kernel (DataProcessingCore and DPFClientAPI binaries), that enables capabilities by loading libraries of operators
(for example, Ans.Dpf.FEMutils, Ans.Dpf.Native and mapdlOperatorsCore libraries are basic libraries enabled by DPF).
This application is also called a **DPF Server application**.

When starting a DPF application, you can to customize the list of operator's libraries that the kernel loads. To learn more on how to customize the initialization of a DPF application, see [Using DPF XML Files](dpf-xml-files.md).
...
- C++ based. For more information on how to write a C++ plugin, see [Creating a DPF custom operator's library](create-cpp-plugin.md).
...
![](../images/dpf-reports.png)

For more information about CPython DPF APIs, see [pydpf-core documentation](https://dpf.docs.pyansys.com/version/stable/).

DPF Standalone capabilities can also be accessed using **C++**. For more information, see [Using DPF capabilities in an existing project](use-dpf-in-existing.md) and [Creating a DPF custom operator's library](create-cpp-plugin.md).

### Mechanical

**DPF in Mechanical** uses IronPython and is accessible via the ACT console. Use it to perform custom postprocessing and visualization of results directly within the Mechanical application.
