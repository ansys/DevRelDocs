# <a id="VM_Managed_DAFUL_GearTrain_Operation_SetFEDesignDocument"></a> Class SetFEDesignDocument
...
```csharp
public abstract class SetFEDesignDocument : SetMeshDesignDocument, IOperation
```
...
```csharp
public SetFEDesignDocument(IMeshEntity meshEntity)
```
...
```csharp
public SetFEDesignDocument(JournalParameter jp)
```
