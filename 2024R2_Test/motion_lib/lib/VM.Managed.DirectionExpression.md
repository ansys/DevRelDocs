# <a id="VM_Managed_DirectionExpression"></a> Class DirectionExpression
...
```csharp
public class DirectionExpression : DirectionBase, IObservableObject, IDisposableObject, ILinkable, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, ILinkContainerEvent, IVerifiable
```
...
```csharp
public DirectionExpression(VectorBase vecPosition)
```
...
```csharp
public void Set(string strDirection)
```
