# <a id="VM_Managed_DAFUL_Car_TestRigDocumentBase"></a> Class TestRigDocumentBase
...
```csharp
public TestRigDocumentBase(UIntPtr docCore)
```
...
```csharp
protected void AddErrorMessageToOutputWindow(string strError)
```
...
```csharp
protected Document3D AddSubSystem(string strFileName)
```
...
```csharp
protected void AddSubSystemForOther(string strFileNames)
```

#### Parameters

`strFileNames` [string](https://learn.microsoft.com/dotnet/api/system.string)
...
```csharp
protected bool BindInterfaces(ObjectBase obFrom, ObjectBase obTo)
```
...
```csharp
protected void ReSetInterfacetable(ObjectBase target, ObjectBase tool)
```
...
```csharp
public virtual void ReplaceAssembly(string strAssemblyInfo)
```
...
```csharp
protected virtual void SetTemplateInfoImpl(DocFromTemplateInfo docTemplateInfo)
```
