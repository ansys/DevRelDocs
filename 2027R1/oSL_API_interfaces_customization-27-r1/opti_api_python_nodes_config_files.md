...
| EnableReadMode| Enables read mode for [Node Plugins](opti_api_python_nodes_basic.md), [Integration Node Plugins](opti_api_python_nodes_integration.md), and [MOP Node Plugins](opti_api_python_nodes_mop.md). |
...
| StdErrIndicatesError | Specifies whether printing to the Standard Error Stream results in a critical error in optiSLang. For example, some tools print deprecation warnings to the Standard Error Stream; setting this to `false` causes optiSLang to ignore such messages.<br>Defaults to `true`. |
