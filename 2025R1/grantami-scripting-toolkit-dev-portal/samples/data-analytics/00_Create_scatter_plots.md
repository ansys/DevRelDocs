...
training_db.unit_system = 'SI (Consistent)'
...
First generate the list of dictionaries, and then use this list to create a DataFrame. Here, we are using the `Record.record_guid` property as the DataFrame index, but any column with a unique value for each record can be used.
