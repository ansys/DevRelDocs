# <a id="VM_Managed_DAFUL_DOE"></a> Namespace VM.Managed.DAFUL.DOE

### Namespaces

 [VM.Managed.DAFUL.DOE.Operation](VM.Managed.DAFUL.DOE.Operation.md)

 [VM.Managed.DAFUL.DOE.Result](VM.Managed.DAFUL.DOE.Result.md)

### Classes

 [DOEInformationBase](VM.Managed.DAFUL.DOE.DOEInformationBase.md)

 [DOESimulationAttribute](VM.Managed.DAFUL.DOE.DOESimulationAttribute.md)

 [DesignArrayMaker](VM.Managed.DAFUL.DOE.DesignArrayMaker.md)

 [DesignParameterCase](VM.Managed.DAFUL.DOE.DesignParameterCase.md)

 [DesignParameterInfo](VM.Managed.DAFUL.DOE.DesignParameterInfo.md)

 [PerformanceIndexInfo](VM.Managed.DAFUL.DOE.PerformanceIndexInfo.md)

### Enums

 [DOEType](VM.Managed.DAFUL.DOE.DOEType.md)

 [ResultType](VM.Managed.DAFUL.DOE.ResultType.md)

 [DesignParameterInfo.ValueType](VM.Managed.DAFUL.DOE.DesignParameterInfo.ValueType.md)

