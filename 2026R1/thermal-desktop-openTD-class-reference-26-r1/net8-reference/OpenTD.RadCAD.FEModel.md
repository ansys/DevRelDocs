# <a id="OpenTD_RadCAD_FEModel"></a> Namespace OpenTD.RadCAD.FEModel

### Classes

 [CoordinateSystem](OpenTD.RadCAD.FEModel.CoordinateSystem.md)

Lightweight coordinate system for use creating a finite element
mesh using an FEMeshImporter only.

 [Domain](OpenTD.RadCAD.FEModel.Domain.md)

Lightweight domain for use creating a finite element
mesh using an FEMeshImporter only.

 [EdgeDomain](OpenTD.RadCAD.FEModel.EdgeDomain.md)

Lightweight edge domain for use creating a finite element
mesh using an FEMeshImporter only.

 [EdgeSpec](OpenTD.RadCAD.FEModel.EdgeSpec.md)

Lightweight edge for use creating a finite element
mesh using an FEMeshImporter only.

 [Element](OpenTD.RadCAD.FEModel.Element.md)

Lightweight element for use creating a finite element
mesh using an FEMeshImporter only.

 [FEMesh](OpenTD.RadCAD.FEModel.FEMesh.md)

Lightweight mesh for use creating a finite element
mesh using an FEMeshImporter only.

 [FaceDomain](OpenTD.RadCAD.FEModel.FaceDomain.md)

Lightweight face domain for use creating a finite element
mesh using an FEMeshImporter only.

 [FaceSpec](OpenTD.RadCAD.FEModel.FaceSpec.md)

Lightweight face for use creating a finite element
mesh using an FEMeshImporter only.

 [IdDomain](OpenTD.RadCAD.FEModel.IdDomain.md)

Lightweight domain ID holder for use with FEMeshImporter only.

 [LayerData](OpenTD.RadCAD.FEModel.LayerData.md)

Lightweight layer for use creating a finite element
mesh using an FEMeshImporter only.

 [Node](OpenTD.RadCAD.FEModel.Node.md)

Lightweight node for use creating a finite element
mesh using an FEMeshImporter only.

 [Quaternion](OpenTD.RadCAD.FEModel.Quaternion.md)

Lightweight quaternion for use creating a finite element
mesh using an FEMeshImporter only.

 [SolidElement](OpenTD.RadCAD.FEModel.SolidElement.md)

Lightweight solid element for use creating a finite element
mesh using an FEMeshImporter only.

 [SurfaceElement](OpenTD.RadCAD.FEModel.SurfaceElement.md)

Lightweight surface element for use creating a finite element
mesh using an FEMeshImporter only.

