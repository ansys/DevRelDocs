...
6. In the solver files directory, commands are written to two python files: `filepre.py` and `filepost.py`. `filepre.py` contains the commands that are executed before the solve (all command snippets except those at solution level). `filepost.py` is executed after the solve (only command snippets at solution level).

7. Open the desired file in Visual Studio. You can insert breakpoints as required
  and click Ignore in the warning dialog to resume the solve.
