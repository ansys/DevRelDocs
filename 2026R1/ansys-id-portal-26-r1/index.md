# Ansys ID Portal sync — setup and usage guide

**Related documentation:** [Entra ID app registration (Azure Portal)](entra_setup.md) — create the Graph app and secrets once; then use this guide to install and run the scripts.
...
Two scripts are provided that perform identical synchronization:
  ...
  - sync_entra_to_ansys.py  -- Python, runs on Linux or Windows
  - sync_entra_to_ansys.ps1 -- PowerShell, runs on Windows only
