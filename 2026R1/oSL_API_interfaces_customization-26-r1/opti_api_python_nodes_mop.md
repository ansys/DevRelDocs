...
- Use the following naming convention for the script files: `*_cmn.py`.
...
- Place the scripts in a directory scanned by optiSLang. Custom nodes found in any of these locations are loaded when the application starts and are then available within optiSLang. There are several location options:
    - The default custom node plugin folder belonging to the installation: `[installation path]/scripting/mop_node`
...
**Incoming Arguments**
- [Common Incoming Key Word Arguments (KWARGs)](#table-15-common-incoming-key-word-arguments-kwargs)
- [Additional Incoming KWARGs for the Execute Function](#table-16-additional-incoming-kwargs-for-the-execute-function)
...
You can find a simple demo exploiting the initialize-shutdown mechanism in the [integration examples](opti_api_python_nodes_integration_demos.md) package.
...
- [HidSpecificInputData](#table-19-hidspecificinputdata-attributes): An aggregate of various input data which may differ for each Hierarchical ID (HID).
- [HidSpecificOutputDataMop](#table-20-hidspecificoutputdatamop-attributes): An aggregate of various output data which may differ for each HID.
- RunStatus: An object for documenting success and logging.
...
| resulting_parameter_manager | PyParameterManager | An object for storing and handling parameters. |
