...
![RosenSuzukiEvaluator](../../../../../../graphics/class_phoenix_1_1_optimization_1_1_algorithm_tests_1_1_evaluators_1_1_rosen_suzuki_evaluator.png)

## Declaration

```csharp
class RosenSuzukiEvaluator
```
...
```
f(x) = x1^2 + x2^2 + 2 * x3^2 + x4^2 - 5 * x1 - 5 * x2 - 32 * x3 + 7 * x4
```
The nonlinear constraints are:
```
0 <= 8 - x1^2 - x2^2 - x3^2 - x4^2 - x1 + x2 - x3 + x4

0 <= 10 - x1^2 - 2 * x2^2 - x3^2 - 2 * x4^2 + x1 + x4

0 <= 5 - 2 * x1^2 - x2^2 - x3^2 - 2 * x1 + x2 + x4
```
...
Adds constraints. Reimplemented from [`ModelEvaluator`](ModelEvaluator.md).
