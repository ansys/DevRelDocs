# FrankEvaluator Class

**Namespace:** `Phoenix.Optimization.AlgorithmTests.Evaluators`

## Overview

Implementation of Frank Fucntion Source: Phoenix Integration Customer  
 

## Inheritance

![FrankEvaluator](../../../../../../graphics/class_phoenix_1_1_optimization_1_1_algorithm_tests_1_1_evaluators_1_1_frank_evaluator.png)

## Declaration

```csharp
class FrankEvaluator
```

## Description

Implementation of Frank Function Source: Phoenix Integration Customer  
```
Number of variables: 2 variables. Definition: f(x,y,z) = (x - 1.0) * (x - 1.0) - (y - 2.0) * (y - 2.0) * (y - 2.0) + 3.0 * x * y + y * z + 6.0 * x

Subject to: g(x,y,z) <= 0

Where: g(x,y,z) = (z - 3.0) * (z - 3.0) + (x - 1.0) * (y - 2.0) - 2 * x * z + 5 * y

Search domain: 0 < x < 5 -2 < y < 2 1 < z < 7

Number of local minima: no local minima except the global one.

The global minima: x= 0.444395722 y= 0.380031748 z= 3.192122344)

f(x, y, z) = 8.946109775 g(x, y, z) = 0 
```
...
|Name|Description|
|-----|-----|
|override object[,] | `GlobalBestDesigns [get]`<br>Global best designs for the evaluator. |
