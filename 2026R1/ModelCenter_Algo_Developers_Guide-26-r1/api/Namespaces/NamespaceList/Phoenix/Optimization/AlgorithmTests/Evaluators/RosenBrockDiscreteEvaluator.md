# RosenBrockDiscreteEvaluator Class
...
![RosenBrockDiscreteEvaluator](../../../../../../graphics/class_phoenix_1_1_optimization_1_1_algorithm_tests_1_1_evaluators_1_1_rosen_brock_discrete_evaluator.png)

## Declaration

```csharp
class RosenBrockDiscreteEvaluator
```
...
Implementation of Rosenbrock Fucntion Source: http://www-optima.amp.i.kyoto-u.ac.jp/member/student/hedar/Hedar_files/TestGO_files/TestGO_files/TestCodes/rosen.m
```
Number of variables: variable (default is 2) Definition: f(x) = sum (i = 0 to n) [100 * ((x_i)^2 - (x_(i+1))^2)^2 + (x_i - 1)^2] Search domain: -2 <= xi <= 2, i = 1, 2,..., n. Discrete choices with 0.05 increment Number of local minima: several local minima. The global minima: x* = (1, ..., 1), f(x*) = 0. 
```
...
### RosenBrockDiscreteEvaluator()
```csharp
RosenBrockDiscreteEvaluator ( )
```

Creates 2 dimensional Rosenbrock evaluator.

### RosenBrockDiscreteEvaluator()
```csharp
RosenBrockDiscreteEvaluator ( int numDimensions)
```

Creates N dimensional Rosenbrock evaluator.

**Parameters:**

- `numDimensions` - the number of dimensions to be used for the Rosenbrock function.
