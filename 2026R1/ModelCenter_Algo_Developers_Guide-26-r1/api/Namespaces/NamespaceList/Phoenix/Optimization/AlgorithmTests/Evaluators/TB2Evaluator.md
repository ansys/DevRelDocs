...
Implementation for the TB2 Function problem  
 

## Inheritance

![TB2Evaluator](../../../../../../graphics/class_phoenix_1_1_optimization_1_1_algorithm_tests_1_1_evaluators_1_1_t_b2_evaluator.png)
...
Source: NonLinear Programming Theory and Algorithms, Second Edition Wiley Student edition ISBN 9812-53-068-1 by Mokhtar S Bazaraa, Hanif D. Sherali and C.M. Shetty Page No. 547, Example 11.30 
```
Minimize e^x1 + x1^2 + 4*x1 + 2*x2^2 - 6*x2 + 2*x3 subject to x1^2 + e*x2 + 6*x3 <= 15 x1^4 - x2 + 5*x3 <= 25 Number of variables: 3 variables Search domain: 0 <= x1 >= 4 0 <= x1 >= 2 0 <= x1 The global minima: x* = {0.0, 1.5, 0.0}, f(x*) = -3.5 
```
...
| |`TB2Evaluator ()`<br>Default constructor uses two design variables |
...
|Name|Description|
|-----|-----|
|override object[,] | `GlobalBestDesigns [get]`<br>The expected result is -3.5 for the minima at (0.0, 1.5, 0) |
...
|override string | `Name [get]`<br>The name of the evaluator. |
...
Default constructor uses two design variables
...
- `designVariables` - Values of design variables to be evaluated.
...
The expected result is -3.5 for the minima at (0.0, 1.5, 0)
