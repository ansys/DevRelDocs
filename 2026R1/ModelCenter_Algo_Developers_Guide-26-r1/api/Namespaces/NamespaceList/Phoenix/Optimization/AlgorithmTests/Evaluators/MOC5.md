...
Implementation of Constrained Multi-objective Optimization problem 2 from CEC'09.  
 

## Inheritance

![MOC5](../../../../../../graphics/class_phoenix_1_1_optimization_1_1_algorithm_tests_1_1_evaluators_1_1_m_o_c5.png)
...
```
Number of variables: 10 variables. Definition: f(x) = f1,f2 Search domain: 0<= x1 <=1; -1<= xi <=1, i=2 to 10;
```
...
|override double |`getIGD (List< double[]> `bestDesignObjectives`, string filePath)`<br>Calculates and returns the IGD value [Implemented only by Multi Objective Evaluators] [Note : Only the file name needs to be changed in this function in the foreach loop] |
...
| |`MOC5 ()`<br>Default constructor, uses the default goal of `minimize` |
| |`MOC5 (Goal goal)`<br>Constructor to use custom goal |
...
Method called to specifiy the constraints for the evaluator (by default, not constraints are specified) Reimplemented from ModelEvaluator.
...
Calculates and returns the IGD value [Implemented only by Multi Objective Evaluators] [Note : Only the file name needs to be changed in this function in the foreach loop] Reimplemented from ModelEvaluator.
