# RosenbrockEvaluator Class

**Namespace:** `Phoenix.Optimization.AlgorithmTests.Evaluators`

## Overview

Implementation of Rosenbrock Fucntion Source: http://www-optima.amp.i.kyoto-u.ac.jp/member/student/hedar/Hedar_files/TestGO_files/TestGO_files/TestCodes/rosen.m


## Inheritance

![RosenbrockEvaluator](../../../../../../graphics/class_phoenix_1_1_optimization_1_1_algorithm_tests_1_1_evaluators_1_1_rosenbrock_evaluator.png)
...
Implementation of Rosenbrock Fucntion Source: http://www-optima.amp.i.kyoto-u.ac.jp/member/student/hedar/Hedar_files/TestGO_files/TestGO_files/TestCodes/rosen.m
```
Number of variables: variable (default is 2) Definition: f(x) = sum (i = 0 to n) [100 * ((x_i)^2 - (x_(i+1))^2)^2 + (x_i - 1)^2] Search domain: -2 <= xi <= 2, i = 1, 2,..., n. Number of local minima: several local minima. The global minima: x* = (1, ..., 1), f(x*) = 0. 
```
...
| |`RosenbrockEvaluator ()`<br>Creates 2 dimensional Rosenbrock evaluator with bounds on the design variables. |
| |`RosenbrockEvaluator (int numDimensions)`<br>Creates N dimensional Rosenbrock evaluator with bounds on the design variables. |
| |`RosenbrockEvaluator (int numDimensions, bool useDesignVariableBounds)`<br>Creates N dimensional Rosenbrock evaluator with the ability to specify whether or not to bound design variables. |
...
Creates 2 dimensional Rosenbrock evaluator with bounds on the design variables.

### RosenbrockEvaluator()
```csharp
RosenbrockEvaluator ( int numDimensions)
```

Creates N dimensional Rosenbrock evaluator with bounds on the design variables.

**Parameters:**

- `numDimensions` - Number of dimensions for the Rosenbrock evaluator.

### RosenbrockEvaluator()
```csharp
RosenbrockEvaluator ( int numDimensions, bool useDesignVariableBounds )
```

Creates N dimensional Rosenbrock evaluator with the ability to specify whether or not to bound design variables.

**Parameters:**

- `numDimensions` - Number of dimensions for the Rosenbrock evaluator.
- `useDesignVariableBounds` - True to bound design variables; false otherwise.
