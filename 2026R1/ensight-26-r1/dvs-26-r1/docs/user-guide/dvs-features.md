...
Sometimes solvers need to rewind to a previous timestep and begin writing new data for various reasons. The delete api facilitates this allowing users to make a call to delete specific timesteps or a collection of timesteps. This call is similar to the [dvs\_begin\_update()](../api/files/dynamic__visualization__store__api_8h.md#function-dvs_begin_update) and [dvs\_end\_update()](../api/files/dynamic__visualization__store__api_8h.md#function-dvs_end_update) calls where all ranks must make the same call for each rank in parallel. The API calls for this are [dvs\_delete\_item()](../api/files/dynamic__visualization__store__api_8h.md#function-dvs_delete_item) (C API), [DVS::IClient::delete\_item()](../api/classes/class_d_v_s_1_1_i_client.md#function-delete_item) (C API), and delete_item() (Python API).
