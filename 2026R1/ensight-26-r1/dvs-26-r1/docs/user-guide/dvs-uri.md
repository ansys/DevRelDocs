...
| `hdf5://localhost:1234/./` | Use `hdf5` cache, localhost/port ignored, cache location in current working directory of server or same directory as `.dvs` file |
...
| --- | --- |
| `query` | Query stanzas to filter what is loaded from the cache. See [Query Stanzas](./dvs-filtering.md#query-stanzas) |
| `compression` | Default: `0`, No compression. `1`: Best compression speed; least compression. `2`-`8`: Compression improves; speed degrades. `9`: Best compression ratio; slowest speed. See [Cache Compression](#cache-compression). |
...
| `hdf5:///./my_cache/?query=/dataset.name/eq/my+dataset//&compression=9` | Max compression with a [Query Stanzas](./dvs-filtering.md#query-stanzas) |
