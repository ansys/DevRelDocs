...
A query in DVS is specified by a number of query stanzas chained together to select a specific set of data entries from a larger set. These stanzas in general are usually AND'd together. The overall structure is in the table below. An individual query stanza has multiple sections separate by a single `/` character and ended with 2 `//` characters. For Examples see [Misc Query Examples](#misc-query-examples).
...
| --- | --- |
| {[Top Level Operator](#top-level-operator)}/{[Objects/Attributes](#objectsattributes)}/{[Operators](#operators)}/{[Operands](#operands)} | The format of a single query stanza |
| {[Top Level Operator](#top-level-operator)}/{[Objects/Attributes](#objectsattributes)}/{[Operators](#operators)}/{[Operands](#operands)}//{[Top Level Operator](#top-level-operator)}/{[Objects/Attributes](#objectsattributes)}/{[Operators](#operators)}/{[Operands](#operands)} | Example of multiple stanzas |
| {[Top Level Operator](#top-level-operator)}/{[Objects/Attributes](#objectsattributes)}/{[Operators](#operators)}/{[Operands](#operands)}\|{[Operands](#operands)} | Multiple operands |
...
The first part of a query stanza is an operator which is used to chain it with the previous stanza. The overall operator portion of the **first** stanza is ignored and optional but must have a `/` separator before the [Objects/Attributes](#objectsattributes) section.
...
Operands can be separated by the `|` character to OR them together for `eq`, `neq` cases. The number of operands allowed per operator is in the [Operators](#operators) table.
