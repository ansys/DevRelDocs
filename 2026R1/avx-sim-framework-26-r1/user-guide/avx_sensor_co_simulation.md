            ...
            },
            {
                ...
                "avx_sensor_setting": {
                    ...
                    "sensor_data_access": {
                        ...
                        "publish_topic_name": "AvxCameraSensorDataTopic",
...
The AVxcelerate Sensor co-simulation provides access to sensor output data through a configurable data access mechanism. This feature allows other activities in the simulation framework to receive and process sensor data generated by the AVxcelerate Sensor service. Additionally, sensor data can be recorded to MCAP files for later playback and analysis.
...
| Parameter            | Type    | Required | Description                                                                                                                     |
| -------------------- | ------- | -------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `enable_publisher`   | boolean | No       | Whether to enable sensor data publishing. Set to `true` to publish sensor data metadata to the specified topic. Default is `false`.  |
| `publish_topic_name` | string  | No       | The name of the topic where sensor data metadata will be published. Other activities can subscribe to this topic to receive sensor data metadata. Only used when `enable_publisher` is `true`. |
| `enable_recorder`    | boolean | No       | Whether to record sensor data to an MCAP file. Set to `true` to enable recording. Default is `false`. See [Recording Sensor Data](#recording-sensor-data) for details. |

## Recording Sensor Data

When `enable_recorder` is set to `true`, the AVxcelerate Sensor activity automatically records all sensor data messages to an MCAP file. The MCAP file is created in the simulation output directory with a filename based on the topic name `publish_topic_name` (e.g., `AvxSensorDataTopic.mcap`).

The recorded MCAP file contains:
- **Topic**: Default mcap topic name is `/avx/sensor_data`
- **Message Type**: `ansys::api::avxcelerate::sensors::v1::sensor_data::SensorData` protobuf messages
- **Format**: MCAP format for efficient storage and playback

The MCAP file is automatically finalized and closed when the simulation completes or the activity is destroyed.

**Performance Notice**: Enabling recording (`enable_recorder: true`) will have an impact on simulation performance. The recording process involves serializing sensor data messages and writing them to disk, which adds I/O overhead and may increase memory usage. For high-frequency sensor data or long-running simulations, consider the trade-off between data capture and simulation performance. Monitor disk I/O and available storage space, as MCAP files can grow large depending on the sensor data payload size and simulation duration.

## Playing Back Recorded Sensor Data

Recorded MCAP files can be played back using the `AvxSensorDataPlayer` utility class. This allows you to replay sensor data offline without requiring a live AVxcelerate Sensor connection. Each recorded message is of type `ansys::api::avxcelerate::sensors::v1::sensor_data::SensorData` and can be used for debugging or analyzing purposes (e.g., inspecting camera images, lidar point clouds, or radar detections after a simulation run).

**Basic usage:**

```cpp
#include <co_sim/avx_sensor/avx_sensor_data_player/avx_sensor_data_player.h>

// Create player and open the recorded MCAP file
simulation_framework::avx_sensor_cosim::AvxSensorDataPlayer player("/path/to/output.mcap");
player.Open();
...
**Important Notes:**
- **Follow API Instructions**: Always follow the usage instructions documented in the Doxygen comments in `avx_sensor_data_player.h`. In particular, call `HasNext()` before calling `GetNext()` to ensure proper message availability checking.
- **Performance Considerations**: AVx Sensor Data messages can have very high payload sizes (e.g., high-resolution camera images, dense lidar point clouds). When processing large messages or high-frequency playback, be aware of:
  - **Memory Usage**: Each `GetNext()` call returns a complete copy of the protobuf message, which may consume significant memory for large sensor data payloads.
  - **Processing Time**: Deserialization and processing of large messages may take considerable time. Consider implementing appropriate buffering or streaming strategies for real-time applications.
  - **I/O Performance**: Reading from large MCAP files may involve significant disk I/O, especially when seeking or resetting playback position.

For detailed API documentation and usage guidelines, see the Doxygen comments in `avx_sensor_data_player.h`.

## Data format

When sensor data publishing is enabled (`enable_publisher` is `true`), the AVxcelerate Sensor activity publishes sensor data to the specified topic in the following format:
