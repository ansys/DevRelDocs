...
```markdown
@defgroup group_dvs_uri DVS URIs
@ingroup dvs_main
@brief Description of various URIs used within the DVS project.
@ingroup group_dvs_uri
@tableofcontents

## URI Overview

URIs are used in the DVS API and @ref dvs_file to specify protocol's, locations, and options. They are mainly used during server creation to specify the communication protocol being used, where to connect, and in the DVS Cache to determine what type of cache to use, where it is located, filter specifics on what to load or not load from the cache, and compression levels if needed.

### Table of Contents
- @ref dvs_server_uri
- @ref dvs_cache_uri
  - @ref dvs_cache_uri_domain
  - @ref dvs_cache_uri_hostname
  - @ref dvs_cache_uri_path
  - @ref dvs_cache_uri_options
  - @ref dvs_cache_uri_example
  - @ref dvs_cache_compression

\tableofcontents
***

***
@subsection dvs_server_uri Server URIs

The Server URI specifies what transport protocol to use, the machine and port to connect to if using a network connection. This is used by dvs_server_create() or DVS::CREATE_SERVER_INSTANCE() to create a server to connect to.<br>
...
***
***
@subsection dvs_cache_uri Cache URIs

Cache URIs are used to specify what domain/type of @ref dvs_cache to use, the host and port of the machine to connect if needed, the path of the cache on the machine, filters and options. Cache URIs are used by both the DVS API if wanting to cache data or a @ref dvs_file to load data previously cached. The URI is set via the API by calling `dvs_server_set_option()` in tha C API or `DVS::IServer::set_option()` in the C API, using "CACHE_URI" as the key, and the URI as the value.<br>
...
***
@subsubsection dvs_cache_uri_domain Cache URI Domain
...
***
@subsubsection dvs_cache_uri_hostname Cache URI Hostname/Port
...
***
@subsubsection dvs_cache_uri_path Cache URI Path
The path in the cache URI can be relative or absolute. A relative path in a URI for a server option will store the cache relative to the working directory of the server data is being sent to. A relative path in a @ref dvs_file will be relative to the location of the file itself.
...
hdf5://localhost:1234/./                           | Use hdf5 cache, localhost/port ignored, cache location in current working directory of server or same directory as @ref dvs_file
hdf5://localhost:1234//home/user/my_cache/         | Use hdf5 cache, localhost/port ignored, cache location /home/user/my_cache on Linux

***
@subsubsection dvs_cache_uri_options Cache URI Options
...
----------------- | -----------
query             | Query stanzas to filter what is loaded from the cache. See @ref dvs_query_stanzas
compression       | Default: 0, No compression. 1: Best compression speed; least compression. 2-8: Compression improves; speed degrades. 9: Best compression ratio; slowest speed. See @ref dvs_cache_compression.

***
@subsubsection dvs_cache_uri_example Cache URI Examples
...
***
@subsubsection dvs_cache_compression Cache Compression
...
hdf5:///./my_cache/?query=/dataset.name/eq/my+dataset//&compression=9 | Max compression with a @ref dvs_query_stanzas
```
