# File dvs\_filtering.md

<a id="dvs__filtering_8md"></a>
...
```markdown
@defgroup group_dvs_filtering DVS Filtering
@ingroup dvs_main
@brief Description of filtering of data within the DVS project
@ingroup group_dvs_filtering
@tableofcontents


Data selection of the current view of data is done via a chained set of filters (@ref dvs_query_stanzas). These filters make it possible to dynamically down select to a subset of data in the cache. This can be used to filter data within a single cache and in the future form multiple DVS data sources at the same time. This will enable the creation of ensemble views of multiple disparate DVS data sources, allowing users to combine design points or even data from different domains (i.e. mechanical and fluids) into one overall view to iterate over. Allowing more powerful and interesting postprocessing of the data for comparison.

These filters are used in the @ref dvs_query_stanzas section of the @ref dvs_cache_uri and also in the DVS Reader API DVS::IQuery.

In @ref dvs_cache_uri the filters are specified by adding a query option to the @ref dvs_cache_uri_options section of the @ref dvs_cache_uri.

The specifics of these filters are in the sections below.

***
### Table of Contents
- @ref dvs_query_stanzas
    - @ref dvs_query_top_level_operator
    - @ref dvs_query_objects
    - @ref dvs_query_operator
    - @ref dvs_query_operands
- @ref dvs_query_examples

\tableofcontents
***

***
@subsection dvs_query_stanzas Query Stanzas

A query in DVS is specified by a number of query stanzas chained together to select a specific set of data entries from a larger set. These stanzas in general are usually AND'd together. The overall structure is in the table below. An individual query stanza has multiple sections separate by a single '/' character and ended with 2 '//' characters. For Examples see @ref dvs_query_examples.
...
------------------- | -----------
{@ref dvs_query_top_level_operator}/{@ref dvs_query_objects}/{@ref dvs_query_operator}/{@ref dvs_query_operands}                            | The format of a single query stanza
{@ref dvs_query_top_level_operator}/{@ref dvs_query_objects}/{@ref dvs_query_operator}/{@ref dvs_query_operands}//{@ref dvs_query_top_level_operator}/{@ref dvs_query_objects}/{@ref dvs_query_operator}/{@ref dvs_query_operands}                                                                      | Example of multiple stanzas
{@ref dvs_query_top_level_operator}/{@ref dvs_query_objects}/{@ref dvs_query_operator}/{@ref dvs_query_operands}\|{@ref dvs_query_operands} | Multiple operands
...
> "Select Dataset 1" AND "Select Parts with name Inlet*" OR "Select Dataset 2" AND "Select Parts with name Wall*"
The or would split the statements to be wrapped with "()" so it would effectively be:
> ("Select Dataset 1" AND "Select Parts with name Inlet*") OR ("Select Dataset 2" AND "Select Parts with name Wall*").
This should hopefully make the intents clearer for future iterations.

@subsubsection dvs_query_top_level_operator Top Level Operator

The first part of a query stanza is an operator which is used to chain it with the previous stanza. The overall operator portion of the **first** stanza is ignored and optional but must have a '/' separator before the @ref dvs_query_objects section.
...
@subsubsection dvs_query_objects Objects/Attributes
...
@subsubsection dvs_query_operator Operators
...
@subsubsection dvs_query_operands Operands
Operands can be separated by the '|' character to OR them together for eq, neg cases. The number of operands allowed per operator is in the @ref dvs_query_operator table.
...
***
@subsection dvs_query_examples Misc Query Examples
