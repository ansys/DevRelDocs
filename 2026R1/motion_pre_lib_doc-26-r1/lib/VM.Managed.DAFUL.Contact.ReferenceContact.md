...
<a id="VM_Managed_DAFUL_Contact_ReferenceContact"></a>
...
```csharp
public class ReferenceContact : Reference, IObservableObject, IDisposableObject, ILinkable, IObjectBase, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, IHasKeyObject, ILinkContainerEvent, IVerifiable, IHistoryObjectSerializable, IAttributeContainer, IReferencable, INamed, INavigatorItem, IVisible, IReferenceAutoContactable, IAutoContactable, IBody, IGapSupport
```
...
```csharp
protected ReferenceContact(SerializationInfo info, StreamingContext context)
```
...
```csharp
public ReferenceContact(ObjectBase obj)
```
