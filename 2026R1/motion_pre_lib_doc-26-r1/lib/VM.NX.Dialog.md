# Namespace VM.NX.Dialog
<a id="VM_NX_Dialog"></a>

### Namespaces

 [VM.NX.Dialog.Factory](VM.NX.Dialog.Factory.md)

### Classes

 [NXFormBase](VM.NX.Dialog.NXFormBase.md)

 [NXFormInitializer](VM.NX.Dialog.NXFormInitializer.md)

 [OperationDialogBase](VM.NX.Dialog.OperationDialogBase.md)

### Interfaces

 [IAppHandle](VM.NX.Dialog.IAppHandle.md)

 [IDlgManager](VM.NX.Dialog.IDlgManager.md)

 [IDocumentDlg](VM.NX.Dialog.IDocumentDlg.md)

 [INXFormInit](VM.NX.Dialog.INXFormInit.md)

 [IPropDlg](VM.NX.Dialog.IPropDlg.md)

 [IPropPage](VM.NX.Dialog.IPropPage.md)

 [IStdDialog](VM.NX.Dialog.IStdDialog.md)

 [IStdModelessDialog](VM.NX.Dialog.IStdModelessDialog.md)

