# Class ApplyShaftSet
<a id="VM_Managed_DAFUL_GearTrain_Operation_ApplyShaftSet"></a>
...
```csharp
public class ApplyShaftSet : ApplyDTEntity, IOperation
```
...
```csharp
public ApplyShaftSet(GearTrainDocument gearTrainDocument, BodyToDTData[] arData, bool bRefresh)
```
...
```csharp
public ApplyShaftSet(JournalParameter jp)
```
