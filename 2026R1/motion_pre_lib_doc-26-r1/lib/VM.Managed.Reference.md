...
```csharp
public class Reference : Object<ObjectEventCore>, IObservableObject, IDisposableObject, ILinkable, IObjectBase, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, IHasKeyObject, ILinkContainerEvent, IVerifiable, IHistoryObjectSerializable, IAttributeContainer, IReferencable, INamed, INavigatorItem, IVisible, IReferenceAutoContactable, IAutoContactable, IBody
```
...
```csharp
public TMatrix FullTransformation { get; }
```
...
```csharp
public TMatrix Transformation { get; }
```
...
```csharp
protected TMatrix GetFullTransform(Unit unitToConvert)
```
...
```csharp
protected virtual bool GetModelNavigatorInformationImpl(XmlDocument xmlDom, XmlElement xmlRootEle, bool bHasChild)
```
...
```csharp
protected string GetTotalSubSystemName(Document3D doc, ObjectBase obObject)
```
...
`obObject` [ObjectBase](VM.Managed.ObjectBase.md)
...
`docSS` [Document3D](VM.Managed.Document3D.md)
...
```csharp
protected TMatrix GetTransform(Unit unitToConvert)
```
...
```csharp
public virtual bool IsOwner(Reference A_0)
```

#### Parameters

`A_0` [Reference](VM.Managed.Reference.md)
