...
<a id="VM_Managed_DAFUL_SubSystemAssembly"></a>
...
```csharp
public class SubSystemAssembly : Assembly, IObservableObject, IDisposableObject, ILinkable, IObjectBase, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, IHasKeyObject, ILinkContainerEvent, IVerifiable, IHistoryObjectSerializable, IAttributeContainer
```
...
```csharp
protected SubSystemAssembly(SerializationInfo info, StreamingContext context)
```
...
```csharp
public SubSystemAssembly(SubSystemDocument doc)
```
