...
<a id="VM_Managed_DAFUL_GearTrain_Operation_UpdateSlidingBearing"></a>
...
```csharp
public class UpdateSlidingBearing : OperationInHiddenDocument, IOperation
```
...
```csharp
public UpdateSlidingBearing(Document3D document3D)
```
...
```csharp
public UpdateSlidingBearing(JournalParameter jp)
```
