...
```csharp
public class CreateSubEntityOperation : OperationBase, IOperation
```
...
```csharp
public CreateSubEntityOperation(JournalParameter jp)
```

#### Parameters

`jp` [JournalParameter](VM.Managed.Operation.JournalParameter.md)
...
```csharp
public CreateSubEntityOperation(Document doc, string strPName, string strXml, Type type)
```
