# Class Ortho
<a id="VM_Managed_Material_Ortho"></a>
...
```csharp
public class Ortho : MaterialBase, IObservableObject, IDisposableObject, ILinkable, ILinkContainerEvent, IVerifiable, IHistoryObjectSerializable, IAttributeContainer, ISubEntity, IEntityBase, IObjectBase, IObject, ILinkContainer, IOwned, IHasID, IEventProvider, IHasKeyObject, IHasName, IEnabled, IHasComment, INamed, INavigatorItem, IMaterial, ITemplateObject, IArgument
```
...
Name of the orthotropic material.
...
```csharp
public ExpressionValueVariable ShearModulusXY { get; set; }
```
...
```csharp
public ExpressionValueVariable ShearModulusXZ { get; set; }
```
...
```csharp
public ExpressionValueVariable YoungsModulusY { get; set; }
```
